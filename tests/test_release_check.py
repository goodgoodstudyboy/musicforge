...
def test_v127_unified_release_program_continuity_acceptance_board_smoke(tmp_path: Path) -> None:
    ok, detail = _v127_unified_release_program_continuity_acceptance_board_smoke(tmp_path)

    assert ok is True, detail
    assert "board=ready_for_signoff" in detail
    assert "verify=passed" in detail
    assert "standalone=passed" in detail
    assert "declared_extra=failed" in detail
    assert "full_resign_signed_by=failed" in detail
