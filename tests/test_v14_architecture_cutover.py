...
def test_v14_phase_architecture_ratchet_passes_without_hiding_debt() -> None:
    ...
    assert report["metrics"]["active_to_compatibility_import_count"] == 0
    ...
    assert report["metrics"]["active_to_compatibility_by_context"].get("quality", 0) == 0
    assert report["metrics"]["active_to_compatibility_by_context"].get("delivery", 0) == 0
    assert report["metrics"]["active_to_compatibility_by_context"].get("trust", 0) == 0
    assert report["metrics"]["active_to_compatibility_by_context"].get("program", 0) == 0
    assert report["metrics"]["active_dynamic_forwarding_count"] == 0
