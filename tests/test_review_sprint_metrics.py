...
def test_sprint_metrics_candidate_funnel_queue_provider_quality_and_store(tmp_path):
    ...
    assert report["provider_usage"]["total_tokens"] == 15
    ...
    assert summary["provider_tokens"] == 15
...
def _add_applied_version(project_document, task, candidate, candidate_plan, tmp_path: Path):
    ...
    write_json(child_dir / "data" / "edit-metadata.json", {"review_task": {"task_id": task.task_id}, "review_candidate": {"candidate_id": candidate.candidate_id}})
