...
from song_agent.release_portfolio_governance_archive_verifier import release_portfolio_governance_archive_verification_summary
from song_agent.release_portfolio_governance_signoff import (
    ReleasePortfolioGovernanceSignoffStore,
    governance_archive_manifest_hash,
    governance_archive_manifest_integrity_ok,
    governance_change_request_hash,
    governance_change_request_integrity_ok,
    governance_signoff_hash,
    governance_signoff_integrity_ok,
    governance_signoff_summary,
)
from song_agent.release_portfolio_governance_verifier import release_portfolio_governance_verification_summary
from song_agent.releases import stable_hash


PORTFOLIO_GOVERNANCE_AUDIT_SCHEMA_VERSION = 1
PORTFOLIO_GOVERNANCE_AUDIT_EXPORT_SCHEMA_VERSION = 1
PORTFOLIO_GOVERNANCE_AUDIT_BLOCKED_KEYS = DEFAULT_BLOCKED_METADATA_KEYS - {"path"}
AUDIT_ENTRY_HASH_EXCLUDE_KEYS = {"entry_hash"}
AUDIT_REPORT_HASH_EXCLUDE_KEYS = {"integrity_hash", "generated_at", "updated_at"}
AUDIT_MANIFEST_HASH_EXCLUDE_KEYS = {"integrity_hash", "created_at", "updated_at", "zip"}
...
class ReleasePortfolioGovernanceAuditStore:
    ...
    def _queue_entries(self, portfolio_id: str, queue_id: str, queue: dict[str, Any]) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
    ...
    def _audit_findings(self, portfolio_id: str, entries: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def audit_ledger_integrity_ok(entries: list[dict[str, Any]]) -> bool:
    ...
    for index, entry in enumerate(entries, start=1):
        if int(entry.get("sequence") or 0) != index:
            return False
        if str(entry.get("previous_entry_hash") or "") != previous:
            return False
...
def _queue_summaries(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _archive_summary(entries: list[dict[str, Any]]) -> dict[str, Any]:
...
def _portfolio_summary(portfolio: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
...
def _write_markdown(export_dir: Path, report: dict[str, Any]) -> None:
