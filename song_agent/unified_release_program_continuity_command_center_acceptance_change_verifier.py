    ...
    for name in names:
        ...
        for pattern in SENSITIVE_PATTERNS:
            if pattern.search(data):
                ...
                break
    return _check("urpcccacc_redaction_scan", not offenders, "Archive contains no obvious secrets or local paths.", {"offenders": sorted(set(offenders))})
...
def _finish(checks: list[dict[str, Any]], summary: dict[str, Any], extra: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    report = {"package_type": UNIFIED_RELEASE_PROGRAM_CONTINUITY_COMMAND_CENTER_ACCEPTANCE_CHANGE_VERIFICATION_PACKAGE_TYPE, "status": status, "summary": summary, "checks": checks, "blockers": blockers, "warnings": warnings}
