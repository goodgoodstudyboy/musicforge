    ...
    try:
        with zipfile.ZipFile(zip_path) as archive:
            ...
            for check_id, doc in (
                ...
                ("urpca_cc_current_generation_integrity", generation),
    ...
    for name in names:
        ...
        for pattern in SENSITIVE_PATTERNS:
            if pattern.search(data):
                ...
                break
    return _check("urpca_cc_redaction_scan", not offenders, "Archive contains no obvious secrets or local paths.", {"offenders": sorted(set(offenders))})


def _finish(checks: list[dict[str, Any]], summary: dict[str, Any], extra: dict[str, Any] | None = None) -> dict[str, Any]:
    if extra:
        checks.append(extra)
    blockers = [row["check_id"] for row in checks if row.get("status") == "failed" and row.get("severity", "blocking") == "blocking"]
    warnings = [row["check_id"] for row in checks if row.get("status") == "failed" and row.get("severity") == "warning"]
    status = "failed" if blockers else "warning" if warnings else "passed"
    report = {"package_type": UNIFIED_RELEASE_PROGRAM_CONTINUITY_ACCEPTANCE_CHANGE_VERIFICATION_PACKAGE_TYPE, "status": status, "summary": summary, "checks": checks, "blockers": blockers, "warnings": warnings}
    report["zip_sha256"] = summary.get("zip_sha256")
    report["zip_size_bytes"] = summary.get("zip_size_bytes")
    report["manifest_hash"] = summary.get("manifest_hash")
