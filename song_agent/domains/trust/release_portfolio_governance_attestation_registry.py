...
class ReleasePortfolioGovernanceAttestationRegistryStore:
    ...
    def read_registry(self, portfolio_id: str, *, profile: str = "public_summary", default: DomainDocument | None = None) -> DomainDocument:
    ...
    def get_entry(self, portfolio_id: str, entry_id: str, *, profile: str = "public_summary") -> DomainDocument:
    ...
    def register_current_attestation(self, portfolio_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or now_iso()
            payload = payload or {}
    ...
    def publish_entry(self, portfolio_id: str, entry_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or now_iso()
            payload = payload or {}
            profile = str(payload.get("profile") or "public_summary")
    ...
    def revoke_entry(self, portfolio_id: str, entry_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or now_iso()
            payload = payload or {}
            profile = str(payload.get("profile") or "public_summary")
    ...
    def export_registry(self, portfolio_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or now_iso()
            profile = str((payload or {}).get("profile") or "public_summary")
            registry = self._require_registry(portfolio_id, profile)
...
def registry_integrity_ok(registry: DomainDocument | None) -> bool:
...
def registry_entry_integrity_ok(entry: DomainDocument | None) -> bool:
...
def registry_report_integrity_ok(report: DomainDocument | None) -> bool:
...
def registry_manifest_integrity_ok(manifest: DomainDocument | None) -> bool:
