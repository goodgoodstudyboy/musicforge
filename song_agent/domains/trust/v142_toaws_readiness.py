...
_first_hash = _make_deferred_global('_first_hash')
...
def bind_globals(namespace: dict[str, object]) -> None:
    global TrustOperationsAssuranceWatchSignoffNotFoundError, TrustOperationsAssuranceWatchSignoffStateError, _blocker, _file_record, _first_hash, _fs_path, _history_hash
    global _mkdir, _next_id, _now, _read_json_default, _read_json_required, _read_text, _read_zip_json, _safe_id
    global _sanitize, _sha256, _walk_files, _write_json, _write_readme, _write_zip, _zip_entries, entry
    global item, path, row
    ...
    _first_hash = namespace.get('_first_hash', _first_hash)
...
class TrustOperationsAssuranceWatchSignoffStoreReadinessMixin:
    ...
    def read_closeout(self, queue_id: str, *, default: DomainDocument | None = None) -> DomainDocument:
    ...
    def list_change_requests(self, queue_id: str) -> list[DomainDocument]:
    ...
    def refresh_closeout(self, queue_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or _now()
            payload = payload or {}
    ...
    def sign(self, queue_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or _now()
            payload = payload or {}
    ...
    def approve_change_request(self, queue_id: str, change_request_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or _now()
            payload = payload or {}
    ...
    def reset_signoff(self, queue_id: str, change_request_id: str, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or _now()
            state = self._signoff_state(queue_id)
    ...
    def export_archive(self, queue_id: str, payload: DomainDocument | None = None, *, now: str | None = None) -> DomainDocument:
        with self.lock:
            now = now or _now()
            payload = payload or {}
            signoff = self.read_signoff(queue_id, default={})
    ...
    def verify_archive_zip(self, queue_id: str, payload: DomainDocument | None = None) -> DomainDocument:
    ...
    def _closeout_source(self, queue_id: str, payload: DomainDocument) -> tuple[DomainDocument]:
    ...
    def _closeout_blockers(self, source: DomainDocument, watch_report: DomainDocument, hub_report: DomainDocument, assurance_report: DomainDocument) -> list[DomainDocument]:
    ...
    def _ensure_signoff_current(self, queue_id: str, signoff: DomainDocument, payload: DomainDocument) -> None:
    ...
    def _archive_report(self, queue_id: str, signoff: DomainDocument, closeout: DomainDocument, now: str) -> DomainDocument:
        report: object = {
            "schema_version": TRUST_OPERATIONS_ASSURANCE_WATCH_SIGNOFF_SCHEMA_VERSION,
