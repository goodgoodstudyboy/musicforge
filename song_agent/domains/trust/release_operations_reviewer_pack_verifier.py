...
def release_operations_reviewer_pack_verification_summary(report: DomainDocument) -> DomainDocument:
...
def write_release_operations_reviewer_pack_verification_report(report: DomainDocument, path: Path | str) -> Path:
    return write_json(Path(path), sanitize_metadata(report, blocked_keys=VERIFIER_BLOCKED_KEYS))


def print_release_operations_reviewer_pack_verification_report(report: DomainDocument) -> None:
...
def release_operations_reviewer_pack_verification_exit_code(report: DomainDocument) -> int:
