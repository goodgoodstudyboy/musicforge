...
def planning_simulation_projection(report: DomainDocument | None) -> DomainDocument:
...
def governance_projection(
    version: DomainDocument | None,
    *,
    active: DomainDocument | None = None,
...
def planning_rule_impact_hash(report: DomainDocument | None) -> str:
    data = _as_document(report)
    return stable_hash({key: data.get(key) for key in IMPACT_REPORT_INTEGRITY_FIELDS})


def planning_rule_impact_projection(report: DomainDocument | None) -> DomainDocument:
    data = _as_document(report)
    summary = _as_document(data.get("summary"))
