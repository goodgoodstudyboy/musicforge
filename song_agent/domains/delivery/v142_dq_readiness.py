...
ZIP_SIZE_WARNING_BYTES = _make_deferred_global('ZIP_SIZE_WARNING_BYTES')
...
scan_delivery_payload_for_sensitive_values = _make_deferred_global('scan_delivery_payload_for_sensitive_values')
sprint = _make_deferred_global('sprint')
value = _make_deferred_global('value')

def bind_globals(namespace: dict[str, object]) -> None:
    global ZIP_SIZE_WARNING_BYTES, _sha256, _stable_hash, _validate_relative_path, item, key, scan_delivery_payload_for_sensitive_values
    global sprint, value
    ZIP_SIZE_WARNING_BYTES = namespace.get('ZIP_SIZE_WARNING_BYTES', ZIP_SIZE_WARNING_BYTES)
    ...
    scan_delivery_payload_for_sensitive_values = namespace.get('scan_delivery_payload_for_sensitive_values', scan_delivery_payload_for_sensitive_values)
    sprint = namespace.get('sprint', sprint)
...
def _quality_gate_requires_stems(manifest: DomainDocument) -> bool:
...
def _actual_export_files(project_dir: Path) -> list[DomainDocument]:
...
def _actual_zip_info(project_dir: Path, actual_files: list[DomainDocument]) -> DomainDocument:
...
def _final_version_summary(project_document: ProjectDocument | object) -> DomainDocument:
...
def _version_by_id(project_document: ProjectDocument | object, version_id: str | None) -> object | None:
    if not version_id:
        return None
...
def _final_export_summary(manifest: DomainDocument, manifest_exists: bool, manifest_error: str, expected: list[DomainDocument], actual: list[DomainDocument]) -> DomainDocument:
...
def _zip_summary(zip_info: DomainDocument, actual_files: list[DomainDocument], manifest: DomainDocument) -> DomainDocument:
...
def _quality_gate_summary(manifest: DomainDocument, final_version: DomainDocument) -> DomainDocument:
    gate = _as_document(manifest.get("quality_gate"))
    return sanitize_metadata(
        {
...
def _review_sprint_summary(project_export: DomainDocument | None) -> DomainDocument:
...
def _artifact_integrity_summary(expected: list[DomainDocument], actual: list[DomainDocument]) -> DomainDocument:
...
def _build_checks(
    *,
    final_version: DomainDocument,
    final_export: DomainDocument,
    zip_summary: DomainDocument,
    quality_gate: DomainDocument,
    review_sprint: DomainDocument,
    artifact_integrity: DomainDocument,
    manifest: DomainDocument,
    raw_manifest: DomainDocument,
    project_export: DomainDocument | None,
    report_probe: DomainDocument,
) -> list[DomainDocument]:
...
def _project_export_delivery_probe(project_export: DomainDocument | None) -> DomainDocument:
    if not isinstance(project_export, dict):
        return {}
    return {
...
def _qa_status(project_document: ProjectDocument | object, blockers: list[DomainDocument], warnings: list[DomainDocument]) -> str:
...
def _qa_readiness(status: str, checks: list[DomainDocument]) -> str:
    if status == "not_ready":
        return "no_data"
    if status == "stale":
        return "stale"
...
def _check(check_id: str, failed: bool, severity: str, message: str, count: int | float | None = 0) -> DomainDocument:
    return sanitize_metadata(
        {
            "check_id": check_id,
...
def _check_message(check: DomainDocument) -> str:
...
def _file_missing(artifact: DomainDocument, kind: str) -> bool:
...
def _stem_audio_partial(artifact: DomainDocument) -> bool:
