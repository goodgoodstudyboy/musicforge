...
class AssetStore:
    ...
    def list_assets(self, include_hidden: bool = False, filters: dict[str, Any] | None = None) -> list[CreativeAsset]:
        filters = filters or {}
    ...
    def mark_used(self, asset_refs: list[dict[str, Any]], context: dict[str, Any] | None = None) -> list[dict[str, Any]]:
...
def extract_assets_from_song_plan(plan: SongPlan, source: dict[str, Any], request: dict[str, Any]) -> list[dict[str, Any]]:
...
def resolve_asset_refs(store: AssetStore, raw_refs: Any) -> list[dict[str, Any]]:
    if raw_refs is None:
        return []
...
def asset_refs_snapshot(store: AssetStore, raw_refs: Any, *, captured_at: str | None = None) -> dict[str, Any]:
    refs = resolve_asset_refs(store, raw_refs)
    return {"schema_version": 1, "asset_refs": refs, "captured_at": captured_at or now_iso()}


def asset_prompt_summaries(store: AssetStore, raw_refs: Any) -> list[dict[str, Any]]:
    summaries = []
...
def write_asset_refs_snapshot(run_dir: Path, snapshot: dict[str, Any]) -> Path:
...
def asset_public_dict(asset: CreativeAsset) -> dict[str, Any]:
    data = asset.to_dict()
...
def _with_preview(asset: CreativeAsset, **values: Any) -> CreativeAsset:
...
def _scan_blocked_content(value: Any) -> None:
