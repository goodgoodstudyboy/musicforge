...
class SprintActionItem:
    ...
    priority: int
    reason: str = ""
    recommendation: ImplementationDocument = field(default_factory=dict)
    input: ImplementationDocument = field(default_factory=dict)
    result: ImplementationDocument = field(default_factory=dict)
    error: str | None = None
    ...
    @classmethod
    def from_dict(cls, data: DomainDocument) -> "SprintActionItem":
...
class SprintActionQueue:
    ...
    @classmethod
    def from_dict(cls, data: DomainDocument) -> "SprintActionQueue":
        if not isinstance(data, dict):
...
class ReviewSprintActionQueueStore:
    ...
    def update_queue(self, queue: SprintActionQueue, *, event: str | None = None, payload: DomainDocument | None = None, now: str | None = None) -> SprintActionQueue:
        now = now or now_iso()
    ...
    def append_event(self, queue_id: str, event: str, payload: DomainDocument, *, now: str | None = None) -> None:
    ...
    def read_events(self, queue_id: str) -> list[DomainDocument]:
...
def build_action_queue_from_recommendation_report(
    *,
    project_id: str,
    sprint: ReviewSprint,
    recommendation_report: DomainDocument,
    name: str | None = None,
    settings: DomainDocument | None = None,
    now: str | None = None,
) -> SprintActionQueue:
    now = now or now_iso()
...
def recommendation_report_hash(report: DomainDocument) -> str:
...
def action_queue_summary(queue: SprintActionQueue | DomainDocument | None) -> DomainDocument:
    if queue is None:
        return {}
...
def action_queue_export_summary(queues: list[SprintActionQueue | DomainDocument]) -> DomainDocument:
    if not queues:
        return {}
...
def action_queue_collection_summary(queues: list[SprintActionQueue | DomainDocument]) -> DomainDocument:
    return action_queue_export_summary(queues)


def queue_report_is_stale(queue: SprintActionQueue | DomainDocument, recommendation_report: DomainDocument) -> bool:
    if isinstance(queue, dict):
        queue = SprintActionQueue.from_dict(queue)
