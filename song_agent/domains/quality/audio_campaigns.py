...
class AudioCampaignStore:
    ...
    def create_campaign(self, payload: DomainDocument | None = None) -> DomainDocument:
        payload = payload or {}
    ...
    def list_campaigns(self) -> list[DomainDocument]:
    ...
    def read_campaign(self, campaign_id: str) -> DomainDocument:
    ...
    def refresh_campaign(self, campaign_id: str) -> DomainDocument:
        with self.lock:
    ...
    def link_listening_session(self, campaign_id: str, session_id: str) -> DomainDocument:
        with self.lock:
            campaign = self._read_raw_campaign(campaign_id)
    ...
    def create_fix_sprints(self, campaign_id: str, payload: DomainDocument | None = None) -> DomainDocument:
        payload = payload or {}
        with self.lock:
            campaign = self._refresh_case_snapshots(self._read_raw_campaign(campaign_id), write=True)
            if campaign.get("status") == "signed":
            ...
            created: list[ImplementationDocument] = []
    ...
    def refresh_report(self, campaign_id: str) -> DomainDocument:
        with self.lock:
    ...
    def signoff(self, campaign_id: str, payload: DomainDocument | None = None) -> DomainDocument:
        payload = payload or {}
        with self.lock:
            campaign = self._read_raw_campaign(campaign_id)
    ...
    def export_campaign(self, campaign_id: str) -> DomainDocument:
        with self.lock:
            raw_campaign = self._read_raw_campaign(campaign_id)
    ...
    def build_zip(self, campaign_id: str) -> DomainDocument:
        raw_campaign = self._read_raw_campaign(campaign_id)
        if self._is_signed_campaign(raw_campaign, campaign_id):
            self._assert_signed_snapshot_valid(campaign_id, raw_campaign)
    ...
    def verify_zip(self, campaign_id: str, **kwargs: Any) -> DomainDocument:
        zip_path = self.zip_path(campaign_id)
...
_v142_ac_readiness.bind_globals(globals())
