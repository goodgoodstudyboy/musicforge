...
def verify_package_envelope(
    ...
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "zip_size_bytes": target.stat().st_size if target.exists() and target.is_file() else 0,
        "manifest_hash": None,
    }
    checks.append(build_check(f"{spec.check_prefix}_zip_exists", target.exists() and target.is_file(), "Package ZIP exists."))
    ...
    try:
        with zipfile.ZipFile(target) as archive:
            ...
            allowed_nested = {
                ...
                if name in spec.allowed_nested_entries
                or any(re.fullmatch(pattern, name) for pattern in spec.allowed_nested_patterns)
            }
            disallowed_nested = nested if spec.nested_zip_policy == "deny" else sorted(set(nested) - allowed_nested)
            allowed = set(spec.allowed_entries)
            pattern_matches = {
                name for name in names
                if any(re.fullmatch(pattern, name) for pattern in spec.allowed_entry_patterns)
            }
            extra = sorted(name_set - allowed - pattern_matches) if allowed or spec.allowed_entry_patterns else []
            missing = sorted(set(spec.required_entries) - name_set)
            for group in spec.co_required_entry_groups:
                if name_set & set(group):
                    missing.extend(sorted(set(group) - name_set))
            checks.extend(
                [
            ...
            if spec.manifest_entry in name_set:
                try:
