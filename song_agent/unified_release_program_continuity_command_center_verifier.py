    ...
    for name in names:
        ...
        for pattern in SENSITIVE_PATTERNS:
            if pattern.search(data):
                ...
                break
    return _check("urpccc_redaction", not findings, "Package contains no obvious secrets or local paths.", {"findings": sorted(set(findings))})
...
def _finish(checks: list[dict[str, Any]], summary: dict[str, Any], *extra: dict[str, Any]) -> dict[str, Any]:
    ...
    warnings = [row["check_id"] for row in checks if row.get("status") == "failed" and row.get("severity") != "blocking"]
    report = {
