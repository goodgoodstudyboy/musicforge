    ...
    for name in names:
        ...
        for pattern in SENSITIVE_PATTERNS:
            if pattern.search(data):
                ...
                break
    return _check("urpca_redaction_scan", not offenders, "Archive contains no obvious secrets or local paths.", {"offenders": sorted(set(offenders))})
...
def _finish(checks: list[dict[str, Any]], summary: dict[str, Any], first_check: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    blockers = [row["check_id"] for row in checks if row.get("status") == "failed" and row.get("severity") == "blocking"]
