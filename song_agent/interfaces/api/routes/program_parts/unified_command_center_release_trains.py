        ...
        try:
            ...
            if tail == "/handoffs":
                ...
                if method == "POST":
                    ...
                    self._send_json({"ok": True, **detail, "summary": report.get("summary", {}), "status": report.get("status")}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            if tail.startswith("/handoffs/"):
                ...
                if action == "import-response":
                    ...
                    self._send_json({"ok": result.get("verification", {}).get("status") == "passed", **result, "summary": result.get("verification", {}).get("summary", {}), "status": result.get("response", {}).get("decision")}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
            ...
            if tail == "/signoff":
                ...
                self._send_json({"ok": signoff.get("status") == "signed", "signoff": signoff, "summary": {"signoff_hash": signoff.get("integrity_hash")}, "status": signoff.get("status")}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
            if tail == "/archive":
                if method == "GET":
                    manifest_path = self.unified_command_center_release_train_store.archive_manifest_path(train_id)
                    manifest = _interfaces_api_runtime.read_json(manifest_path) if manifest_path.exists() else {}
