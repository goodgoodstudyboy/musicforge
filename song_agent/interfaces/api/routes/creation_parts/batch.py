    ...
    def _handle_job_route(self, method: str, job_id: str, tail: str) -> None:
        ...
        if tail == "/open-folder":
            if method != "POST":
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            _interfaces_api_runtime.open_folder(run_dir)
        ...
        if tail == "/quality":
            ...
            return
        if tail == "/edit":
            metadata = _interfaces_api_runtime._read_edit_metadata_for_run(run_dir)
            if metadata is None:
        ...
        if tail == "/provider-usage":
            ...
            if not usage_path.exists():
                ...
                return
            self._send_json({"job_id": job.job_id, "usage": _interfaces_api_runtime.read_json(usage_path)})
            return
        if tail == "/events":
            self._send_json({"events": _interfaces_api_runtime._read_events(run_dir / "logs" / "events.jsonl")})
            return
        if tail == "/artifacts":
            self._send_json({"artifacts": _interfaces_api_runtime.discover_artifacts(run_dir)})
            return
        if tail == "/midi":
