...
class CreationRoutesProjectReviewSprint(CreationRouteContext):
    def _handle_project_review_sprint_route_part_01(self, method: str, project_id: str, sprint_id: str, action: str, _split_state):
        ...
        _split_state['sprint_store'] = _interfaces_api_runtime.ReviewSprintStore(project_dir)
        _split_state['task_store'] = _interfaces_api_runtime.ReviewTaskStore(project_dir)
        ...
        if action == 'refresh':
            ...
            self.project_store.append_event(project_id, 'review_sprint_refreshed', {'sprint_id': _split_state['sprint'].sprint_id, 'status': _split_state['sprint'].status})
        ...
        if action == 'close':
            ...
            if force and (not str(_split_state['payload'].get('override_reason') or '').strip()):
                ...
                return (True, None)
            if force:
                closeout_report = _split_state['sprint_store'].write_closeout_report(_split_state['sprint'], _interfaces_api_runtime.mark_closeout_report_forced(closeout_report), now=_interfaces_api_runtime._utc_now())
            signoff = _split_state['sprint_store'].read_signoff(_split_state['sprint'].sprint_id, default={})
            if not signoff:
                signoff = _split_state['sprint_store'].write_signoff(_split_state['sprint'], _interfaces_api_runtime.build_signoff_record(project_id=project_id, sprint=_split_state['sprint'], closeout_report=closeout_report, payload={**_split_state['payload'], 'force': force}, now=_interfaces_api_runtime._utc_now()), now=_interfaces_api_runtime._utc_now())
            ...
            self.project_store.append_event(project_id, event_name, {'sprint_id': _split_state['sprint'].sprint_id, 'forced': force, 'closeout_status': closeout_report.get('status')})
    ...
    def _handle_project_review_sprint_route_part_02(self, method: str, project_id: str, sprint_id: str, action: str, _split_state):
        ...
        if action == 'conflicts':
            ...
            if not _split_state['report']:
                _split_state['report'] = _split_state['sprint_store'].detect_conflicts(_split_state['sprint'], task_store=_split_state['task_store'], parent_plan_hashes=self._review_sprint_parent_plan_hashes(project_id, _split_state['task_store'], _split_state['sprint']), now=_interfaces_api_runtime._utc_now())
        ...
        if action == 'recommendations':
            ...
            self._send_json({'ok': True, 'sprint': _split_state['sprint'].to_dict(), 'recommendation_report': _split_state['report'], 'summary': _interfaces_api_runtime.recommendation_report_summary(_split_state['report'])})
            return (True, None)
        return (False, None)
    ...
    def _handle_project_review_sprint_route_part_03(self, method: str, project_id: str, sprint_id: str, action: str, _split_state):
        ...
        if action == 'metrics-refresh':
            ...
            _split_state['sprint_store'].append_event(_split_state['sprint'].sprint_id, 'review_sprint_metrics_refreshed', {'readiness': (_split_state['report'].get('risk_readiness') or {}).get('readiness') if isinstance(_split_state['report'].get('risk_readiness'), dict) else None}, now=_interfaces_api_runtime._utc_now())
            self.project_store.append_event(project_id, 'review_sprint_metrics_refreshed', {'sprint_id': _split_state['sprint'].sprint_id, 'readiness': (_split_state['report'].get('risk_readiness') or {}).get('readiness') if isinstance(_split_state['report'].get('risk_readiness'), dict) else None})
            self._send_json({'ok': True, 'sprint': _split_state['sprint'].to_dict(), 'metrics_report': _split_state['report'], 'summary': _interfaces_api_runtime.sprint_metrics_summary(_split_state['report'])})
            return (True, None)
        ...
        if action == 'closeout-refresh':
            ...
            self.project_store.append_event(project_id, 'review_sprint_closeout_refreshed', {'sprint_id': _split_state['sprint'].sprint_id, 'status': _split_state['report'].get('status'), 'readiness': _split_state['report'].get('readiness')})
            self._send_json({'ok': True, 'sprint': _split_state['sprint'].to_dict(), 'closeout_report': _split_state['report'], 'summary': _interfaces_api_runtime.closeout_report_summary(_split_state['report'])})
            return (True, None)
        if action == 'signoff':
            if method != 'GET':
    ...
    def _handle_project_review_sprint_route_part_04(self, method: str, project_id: str, sprint_id: str, action: str, _split_state):
        if action == 'action-queues':
            ...
            if method == 'POST':
                _split_state['payload'] = self._optional_json_body()
                if bool(_split_state['payload'].get('refresh_recommendations', True)):
                    _split_state['sprint'], _split_state['_conflict_report'] = self._refresh_review_sprint_state(project_id, _split_state['sprint_store'], _split_state['task_store'], _split_state['sprint'])
                    _split_state['report'] = self._refresh_review_sprint_recommendations(project_id, _split_state['sprint_store'], _split_state['task_store'], _split_state['sprint'])
                else:
                    _split_state['report'] = _split_state['sprint_store'].read_recommendation_report(_split_state['sprint'].sprint_id, default={})
        ...
        if action.startswith('action-queue:'):
            ...
            if queue_action == 'detail':
                ...
                if queue.project_id != project_id or queue.sprint_id != _split_state['sprint'].sprint_id:
                    raise FileNotFoundError(queue_id)
                self._send_json({'ok': True, 'sprint': _split_state['sprint'].to_dict(), 'queue': queue.to_dict(), 'events': queue_store.read_events(queue.queue_id), 'summary': _interfaces_api_runtime.action_queue_summary(queue)})
                return (True, None)
            if queue_action == 'run':
                if method != 'POST':
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, 'Method not allowed.')
                    return (True, None)
                _split_state['payload'] = self._optional_json_body()
                _split_state['result'] = self._run_review_sprint_action_queue(project_id, _split_state['sprint_store'], _split_state['task_store'], _split_state['sprint'], queue_store, queue_id, _split_state['payload'])
                self._send_json(_split_state['result'])
                return (True, None)
            if queue_action == 'archive':
                ...
                self.project_store.append_event(project_id, 'review_sprint_action_queue_archived', {'sprint_id': _split_state['sprint'].sprint_id, 'queue_id': archived.queue_id})
