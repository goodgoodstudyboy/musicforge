...
class CreationRoutesProvider(CreationRouteContext):
    ...
    def _handle_provider_route(self, method: str) -> None:
        if method == "GET":
            config, sources = _interfaces_api_runtime.load_provider_config()
            self._send_json(
                {
                    "configured": _interfaces_api_runtime.provider_configured(config),
                    "config": config.to_public_dict(sources),
                }
            )
            return
        if method == "POST":
            config = _interfaces_api_runtime.save_provider_config_from_dict(self._read_json_body())
            self._send_json(
                {
                    "ok": True,
                    "configured": _interfaces_api_runtime.provider_configured(config),
    ...
    def _handle_provider_reset(self, method: str) -> None:
        if method != "POST":
            self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
            return
        _interfaces_api_runtime.reset_provider_config()
        config, _sources = _interfaces_api_runtime.load_provider_config()
        self._send_json({"ok": True, "configured": _interfaces_api_runtime.provider_configured(config)})
    ...
    def _handle_provider_test(self, method: str) -> None:
        if method != "POST":
            ...
            return
        config, _sources = _interfaces_api_runtime.load_provider_config()
        self._send_json(_interfaces_api_runtime.test_provider_config(config))

    def _handle_renderer_route(self, method: str) -> None:
        if method == "GET":
            config, sources = _interfaces_api_runtime.load_renderer_config()
            self._send_json(
                {
                    "configured": _interfaces_api_runtime.renderer_configured(config),
                    "config": config.to_public_dict(sources),
                }
            )
            return
        if method == "POST":
            config = _interfaces_api_runtime.save_renderer_config_from_dict(self._read_json_body())
            self._send_json(
                {
                    "ok": True,
                    "configured": _interfaces_api_runtime.renderer_configured(config),
    ...
    def _handle_renderer_reset(self, method: str) -> None:
        if method != "POST":
            self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
            return
        _interfaces_api_runtime.reset_renderer_config()
        config, _sources = _interfaces_api_runtime.load_renderer_config()
        self._send_json({"ok": True, "configured": _interfaces_api_runtime.renderer_configured(config)})
    ...
    def _handle_renderer_test(self, method: str) -> None:
        if method != "POST":
            ...
            return
        config, _sources = _interfaces_api_runtime.load_renderer_config()
        self._send_json(_interfaces_api_runtime.test_renderer_config(config))
    ...
    def _handle_prompt_template_route(self, method: str, template_id: str, tail: str) -> None:
        if tail == "":
            if method == "GET":
                try:
                    ...
                    self._send_error(_interfaces_api_runtime.HTTPStatus.NOT_FOUND, "Prompt template not found.")
                    return
        ...
        if tail == "/reset":
            ...
            self._send_json({"ok": True, **self.prompt_template_store.to_response()})
            return
        self._send_error(_interfaces_api_runtime.HTTPStatus.NOT_FOUND, "Prompt template route not found.")
    ...
    def _handle_editor_template_route(self, method: str, template_type: str, template_id: str, tail: str) -> None:
        try:
            ...
            if tail == "/delete":
                ...
                return
            self._send_error(_interfaces_api_runtime.HTTPStatus.NOT_FOUND, "Editor template route not found.")
        except FileNotFoundError:
            self._send_error(_interfaces_api_runtime.HTTPStatus.NOT_FOUND, "Editor template not found.")
        except ValueError as exc:
            self._send_error(_interfaces_api_runtime.HTTPStatus.BAD_REQUEST, str(exc))
    ...
    def _handle_projects_root(self, method: str, query_string: str) -> None:
        if method == "GET":
            ...
            hidden_filter = _interfaces_api_runtime._query_value(query, "hidden")
            q = _interfaces_api_runtime._query_value(query, "q")
            status_filter = _interfaces_api_runtime._query_value(query, "status")
            variant_type = _interfaces_api_runtime._query_value(query, "variant_type")
            documents = [
            ...
            projects = [
                ...
                if _interfaces_api_runtime._project_matches_filters(
                    document,
                    q=q,
    ...
    def _handle_assets_root(self, method: str, query_string: str) -> None:
        ...
        if method == "POST":
            try:
                asset = self.asset_store.create_asset(self._read_json_body(), now=_interfaces_api_runtime._utc_now())
            except ValueError as exc:
                self._send_error(_interfaces_api_runtime.HTTPStatus.BAD_REQUEST, str(exc))
                return
            self._send_json(
    ...
    def _handle_library_rebuild(self, method: str) -> None:
        if method != "POST":
            self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
            return
        index = self.library_index_store.rebuild(self.asset_store, self.reference_store, now=_interfaces_api_runtime._utc_now())
        self._send_json({"ok": True, "index": index.summary()})
