...
class CreationRoutesProjectCandidateGroupsList:
    def _create_project_candidate_group(self, project_id: str, version_id: str, payload: dict[str, Any], *, mark_asset_usage: bool = True) -> _interfaces_api_runtime.Any:
        ...
        patches, provider_snapshot = _interfaces_api_runtime.generate_provider_edit_candidates(
        ...
        group = group_store.create_group(
            ...
            source={
                "parent_version_id": parent.version_id,
                "parent_job_id": parent_job.job_id,
                "song_plan_sha256": _interfaces_api_runtime.song_plan_hash(parent_plan),
        ...
        for patch in patches:
            try:
                result = _interfaces_api_runtime.apply_provider_edit_patch(parent_plan, patch)
                validator = {
                    "status": "passed",
                    "checks": ["provider_edit_patch_schema", "edit_intent_validation", "song_plan_validation"],
                    "checked_at": _interfaces_api_runtime._utc_now(),
                }
                scores = _interfaces_api_runtime.score_provider_edit_candidate(
                    parent_plan=parent_plan,
                ...
                group_store.add_candidate(
                    ...
                    patch=patch.to_dict(),
                    scores={},
                    validator={"status": "failed", "error": str(exc), "checked_at": _interfaces_api_runtime._utc_now()},
                    quality=None,
                    error=str(exc),
                    now=_interfaces_api_runtime._utc_now(),
                )
    ...
    def _handle_project_candidate_group_apply(self, method: str, project_id: str, group_id: str) -> None:
        ...
        try:
            ...
            if group.status == "applied":
                ...
                return
            candidate_id = str(payload.get("candidate_id") or _interfaces_api_runtime._top_ranked_candidate_id(group) or "")
            ...
            if candidate.status != "ready":
                ...
                return
            patch = _interfaces_api_runtime.ProviderEditPatch.from_dict(group_store.read_candidate_patch(group.group_id, candidate.candidate_id))
            candidate_plan = _interfaces_api_runtime.SongPlan.from_dict(group_store.read_candidate_plan(group.group_id, candidate.candidate_id))
            ...
            job = self.store.create_edit_job(
                ...
                candidate=_interfaces_api_runtime._candidate_source_summary(
                    {
                        "candidate_group_id": group.group_id,
                        "candidate_id": candidate.candidate_id,
                        "rank": candidate.rank,
            ...
            self._send_error(_interfaces_api_runtime.HTTPStatus.BAD_REQUEST, str(exc))
            return
        self._send_json({"ok": True, **document.to_dict(), "group": group.to_dict(), "version": version.to_dict(), "job": job.to_dict()}, status=_interfaces_api_runtime.HTTPStatus.ACCEPTED)
