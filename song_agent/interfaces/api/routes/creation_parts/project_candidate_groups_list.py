...
from song_agent.application.http_ports.creation import CandidateGroup, ProjectDocument, ProjectVersion
...
class CreationRoutesProjectCandidateGroupsList(CreationRouteContext):
    def _create_project_candidate_group_part_01(
        self, project_id: str, version_id: str, payload: JsonDocument, mark_asset_usage: bool, _split_state
    ):
        ...
        candidate_count = _as_int(payload.get("candidate_count") or 3)
        ...
        _split_state["asset_snapshot"] = _interfaces_api_runtime.asset_refs_snapshot(
            self.asset_store, payload.get("asset_refs"), captured_at=_interfaces_api_runtime._utc_now()
        )
        asset_prompt_refs = _interfaces_api_runtime.asset_prompt_summaries(self.asset_store, payload.get("asset_refs"))
        _split_state["reference_snapshot"] = _interfaces_api_runtime.reference_refs_snapshot(
            self.reference_store, payload.get("reference_refs"), captured_at=_interfaces_api_runtime._utc_now()
        )
        reference_prompt_refs = _interfaces_api_runtime.reference_prompt_summaries(self.reference_store, payload.get("reference_refs"))
        _split_state["patches"], provider_snapshot = _interfaces_api_runtime.generate_provider_edit_candidates(
            parent_plan=_split_state["parent_plan"],
            instruction=instruction,
            template=_split_state["template"],
            config=config,
        ...
        _split_state["group_store"] = _api_store_factories.candidate_group_store(project_dir)
        _split_state["group"] = _split_state["group_store"].create_group(
            project_id=project_id,
            parent_version_id=_split_state["parent"].version_id,
            parent_job_id=parent_job.job_id,
            instruction=instruction,
            template_id=_split_state["template"].template_id,
            candidate_count=len(_split_state["patches"]),
            source={
                "parent_version_id": _split_state["parent"].version_id,
                "parent_job_id": parent_job.job_id,
                "song_plan_sha256": _interfaces_api_runtime.song_plan_hash(_split_state["parent_plan"]),
                "asset_refs": list(_split_state["asset_snapshot"]["asset_refs"]),
                "reference_refs": list(_split_state["reference_snapshot"]["reference_refs"]),
                **({"context_pack": _as_document(payload.get("context_pack"))} if isinstance(payload.get("context_pack"), dict) else {}),
            },
        ...
        usage_record = _interfaces_api_runtime._provider_usage_record(
            ...
            template_id=_split_state["template"].template_id,
            started_at=_split_state["group"].created_at,
            status="completed",
            provider_usage=provider_usage,
    ...
    def _create_project_candidate_group_part_02(
        self, project_id: str, version_id: str, payload: JsonDocument, mark_asset_usage: bool, _split_state
    ):
    ...
    def _create_project_candidate_group(
        ...
        *,
        ...
        if _split_result[0]:
            return _split_result[1]
        raise RuntimeError("Candidate group creation did not produce a result.")
    ...
    def _handle_project_candidate_group_usage(self, method: str, project_id: str, group_id: str) -> None:
        ...
        try:
            self.project_store.get_project(project_id)
            project_dir = self.project_store.project_dir(project_id)
            _api_store_factories.candidate_group_store(project_dir).read_group(group_id)
            ...
            return
        self._send_json(
            _interfaces_api_runtime.build_provider_usage_report(scope="candidate_group", project_id=project_id, records=records)
        )
    ...
    def _handle_project_candidate_group_apply_part_01(self, method: str, project_id: str, group_id: str, _split_state):
        ...
        _split_state["group_store"] = _api_store_factories.candidate_group_store(self.project_store.project_dir(project_id))
        _split_state["group"] = _split_state["group_store"].read_group(group_id)
        _split_state["parent"] = next(
            (
                _split_state["version"]
                for _split_state["version"] in _split_state["document"].versions
                if _split_state["version"].version_id == _split_state["group"].parent_version_id
            ),
            None,
        )
        if _split_state["parent"] is None:
            ...
            return (True, None)
        _document, _split_state["parent"], _split_state["parent_job"], _split_state["parent_plan"] = self._project_edit_parent(
            project_id, _split_state["parent"].version_id
        )
        if _interfaces_api_runtime.candidate_group_stale(
            _split_state["group"], _interfaces_api_runtime.song_plan_hash(_split_state["parent_plan"])
        ):
            self._send_error(
        ...
        if _split_state["group"].status == "applied":
            ...
            return (True, None)
        candidate_id = str(
            _split_state["payload"].get("candidate_id") or _interfaces_api_runtime._top_ranked_candidate_id(_split_state["group"]) or ""
        )
        ...
        if _split_state["candidate"].status != "ready":
            ...
            return (True, None)
        _split_state["patch"] = _interfaces_api_runtime.ProviderEditPatch.from_dict(
            _split_state["group_store"].read_candidate_patch(_split_state["group"].group_id, _split_state["candidate"].candidate_id)
        )
        candidate_plan = _interfaces_api_runtime.SongPlan.from_dict(
            _split_state["group_store"].read_candidate_plan(_split_state["group"].group_id, _split_state["candidate"].candidate_id)
        )
        candidate_plan.validate()
        _split_state["intent"] = _interfaces_api_runtime.EditIntent.from_dict(
            {
                "edit_type": "section_energy",
                "target": {"section_name": _split_state["parent_plan"].sections[0].name},
                "instruction": _split_state["group"].instruction,
                "strength": 6,
                "provider_mode": "provider",
                "payload": {"candidate_group_id": _split_state["group"].group_id, "candidate_id": _split_state["candidate"].candidate_id},
            }
        )
        config, _sources = _interfaces_api_runtime.load_provider_config()
        _split_state["provider_snapshot"] = config.to_snapshot("provider", _interfaces_api_runtime._utc_now())
        _split_state["usage"] = _interfaces_api_runtime._provider_usage_record(
            config_snapshot=_split_state["provider_snapshot"],
            operation="provider_edit_candidate_apply",
            template_id=_split_state["group"].template_id,
            started_at=_interfaces_api_runtime._utc_now(),
            status="queued",
            provider_usage=_split_state["group"].provider_usage,
            request_id=_split_state["group"].provider_request_id,
        )
        _split_state["name"] = (
            str(_split_state["payload"].get("name") or "") or f"Provider Candidate {len(_split_state['document'].versions) + 1}"
        )
        return (False, None)
    ...
    def _handle_project_candidate_group_apply(self, method: str, project_id: str, group_id: str) -> None:
        ...
        try:
            ...
            self._send_error(_interfaces_api_runtime.HTTPStatus.BAD_REQUEST, str(exc))
            return
        document = cast(ProjectDocument, _split_state["document"])
        group = cast(CandidateGroup, _split_state["group"])
        version = cast(ProjectVersion, _split_state["version"])
        job = cast(JobState, _split_state["job"])
        self._send_json(
            {
                "ok": True,
                **document.to_dict(),
