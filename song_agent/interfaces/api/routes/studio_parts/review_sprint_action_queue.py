...
class StudioRoutesReviewSprintActionQueue(StudioRouteContext):
    def _run_review_sprint_action_queue(
        ...
        selected_ids = _as_list(payload.get("item_ids"))
        ...
        for item in _interfaces_api_runtime._select_action_queue_items(queue, selected_ids, rerun_failed=rerun_failed):
            ...
            except (_interfaces_api_runtime.ReviewSprintStateError, _interfaces_api_runtime.ReviewTaskStateError, _interfaces_api_runtime.ContextPackStaleError) as exc:
        ...
        queue = queue_store.update_queue(_interfaces_api_runtime.replace(queue, status=completed_status), event="queue_run_completed", payload={"result_count": len(results)}, now=_interfaces_api_runtime._utc_now())
        ...
        response.update({"queue": queue.to_dict(), "queue_events": queue_store.read_events(queue.queue_id), "results": _interfaces_api_runtime.sanitize_metadata(results), "action_queue_summary": _interfaces_api_runtime.action_queue_summary(queue)})
        return response
    ...
    def _execute_review_sprint_action_item(
        ...
        if item.action == "refresh_conflicts":
            ...
            return _interfaces_api_runtime.replace(item, status="completed", result={"conflict_count": len(report.get("conflicts", []))}, completed_at=_interfaces_api_runtime._utc_now())
        if item.action == "refresh_recommendations":
            ...
            return _interfaces_api_runtime.replace(item, status="completed", result={"recommended_count": len(report.get("recommended_order", [])), "created_at": report.get("created_at")}, completed_at=_interfaces_api_runtime._utc_now())
        ...
        if _interfaces_api_runtime.queue_report_is_stale(queue, report):
            return _interfaces_api_runtime.replace(item, status="blocked", error="Action queue is stale because the Recommendation Report changed. Recreate the queue.", completed_at=_interfaces_api_runtime._utc_now())
    ...
    def _ensure_action_item_task_current(self, project_id: str, task_store: _InterfaceType, sprint: Any, item: _InterfaceType) -> Any:
    ...
    def _generate_review_task_local_candidates_for_queue(self, project_id: str, task_store: _InterfaceType, task: Any, payload: ImplementationDocument) -> ImplementationDocument:
        ...
        _document, _parent, _parent_job, parent_plan = self._project_edit_parent(project_id, task.parent_version_id)
        _interfaces_api_runtime.ensure_task_current(task, parent_plan)
        strategies = _list_or(payload.get("strategies"), ["balanced"])
        ...
        for candidate, candidate_plan, validator, summary in _interfaces_api_runtime.build_local_review_candidates(task, parent_plan, strategies=strategies):
            generated.append(task_store.create_candidate(task=task, candidate=candidate, candidate_plan=candidate_plan, validator=validator, summary=summary, render_midi_file=render_midi, now=_interfaces_api_runtime._utc_now()))
        ranked = task_store.rank_candidates(task)
        updated_task = task_store.update_counts(task, now=_interfaces_api_runtime._utc_now())
        decision_report = task_store.write_decision_report(updated_task, _interfaces_api_runtime.build_review_decision_report(task=updated_task, candidates=ranked, parent_plan=parent_plan, now=_interfaces_api_runtime._utc_now()), now=_interfaces_api_runtime._utc_now())
    ...
    def _read_review_task_judge_report(self, project_id: str, task_store: _InterfaceType, task: Any, candidates: list[Any] | None = None, *, parent_plan: _InterfaceType | None = None) -> ImplementationDocument:
    ...
    def _refresh_review_task_judge_report(self, project_id: str, task_store: _InterfaceType, task: Any, payload: ImplementationDocument | None = None) -> ImplementationDocument:
        ...
        refreshed_decision = task_store.write_decision_report(
            task,
            _interfaces_api_runtime.build_review_decision_report(task=task, candidates=ranked, parent_plan=parent_plan, now=_interfaces_api_runtime._utc_now(), notes=str(payload.get("decision_note") or ""), judge_report=saved),
            now=_interfaces_api_runtime._utc_now(),
        )
    ...
    def _refresh_review_task_decision_report_for_queue(self, project_id: str, task_store: _InterfaceType, task: Any, payload: ImplementationDocument) -> ImplementationDocument:
        _document, _parent, _parent_job, parent_plan = self._project_edit_parent(project_id, task.parent_version_id)
        _interfaces_api_runtime.ensure_task_current(task, parent_plan)
        ranked = task_store.rank_candidates(task)
        judge_report = self._read_review_task_judge_report(project_id, task_store, task, ranked, parent_plan=parent_plan)
        decision_report = task_store.write_decision_report(task, _interfaces_api_runtime.build_review_decision_report(task=task, candidates=ranked, parent_plan=parent_plan, now=_interfaces_api_runtime._utc_now(), notes=str(payload.get("note") or ""), judge_report=judge_report), now=_interfaces_api_runtime._utc_now())
        ...
        return {"decision_report": _interfaces_api_runtime.review_decision_summary(decision_report), "provider_summary": _interfaces_api_runtime.review_candidate_source_breakdown(ranked), "candidate_count": len(ranked)}
