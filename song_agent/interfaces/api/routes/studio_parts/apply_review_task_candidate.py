...
from song_agent.application.http_ports.studio import EditedSongPlanResult, ProjectVersion, ReviewCandidate, ReviewTask, SongPlan
...
class _ReviewCandidateApplyState:
    job: JobState | None = None
    version: ProjectVersion | None = None
    result: EditedSongPlanResult | None = None

    def completed(self) -> tuple[ProjectVersion, JobState, EditedSongPlanResult]:
        if self.version is None or self.job is None or self.result is None:
            raise RuntimeError("Review candidate apply state is incomplete.")
        return self.version, self.job, self.result


class StudioRoutesApplyReviewTaskCandidate(StudioRouteContext):
    def _apply_review_task_candidate_part_01(
        self,
        project_id: str,
        ...
        state: _ReviewCandidateApplyState,
    ) -> None:
        ...
        state.result = _interfaces_api_runtime.apply_candidate_intents(parent_plan, [_interfaces_api_runtime.EditIntent.from_dict(item) for item in candidate.intents])
        ...
        metadata = {
            **job.edit_metadata,
            **_interfaces_api_runtime.candidate_apply_metadata(task, candidate, state.result, decision_report=decision_report),
        ...
        self._attach_review_candidate_context(
            metadata,
            job.input_payload,
            sprint_membership,
        ...
        write_interface_document(
            _interfaces_api_runtime.ProjectPaths.create(_interfaces_api_runtime.Path(job.output_dir)).data / "edit-metadata.json",
            metadata,
        )
        ...
        state.job = job
        state.version = next(version for version in document.versions if version.job_id == job.job_id)

    @staticmethod
    def _attach_review_candidate_context(
        metadata: JsonDocument,
        input_payload: JsonDocument,
        sprint_membership: JsonDocument,
        sprint_recommendation: JsonDocument,
        sprint_action_queue: JsonDocument,
    ) -> None:
        if sprint_membership:
            metadata["review_sprint"] = sprint_membership
            input_payload["review_sprint"] = sprint_membership
        if sprint_recommendation:
            metadata["review_sprint_recommendation"] = sprint_recommendation
            input_payload["review_sprint_recommendation"] = sprint_recommendation
        if sprint_action_queue:
            metadata["review_sprint_action_queue"] = sprint_action_queue
            input_payload["review_sprint_action_queue"] = sprint_action_queue

    def _apply_review_task_candidate_part_02(
        self,
        project_id: str,
        task_store: studio_ports.ReviewTaskStore,
        task: ReviewTask,
        ...
        version, job, result = state.completed()
    ...
    def _apply_review_task_candidate(
        ...
        state = _ReviewCandidateApplyState()
        self._apply_review_task_candidate_part_01(project_id, task_store, task, candidate, parent, parent_job, parent_plan, payload, state)
        return self._apply_review_task_candidate_part_02(project_id, task_store, task, candidate, parent, parent_job, parent_plan, payload, state)

    def _create_review_task_follow_up(self, project_id: str, task_store: studio_ports.ReviewTaskStore, task: ReviewTask, payload: JsonDocument) -> tuple[ReviewTask, ReviewTask]:
        ...
        preview = _api_store_factories.editor_preview_store(self.project_store.project_dir(project_id)).read_preview(task.preview_id)
        audition_store = _api_store_factories.editor_audition_store(self.project_store.project_dir(project_id))
            ...
            type(task).from_dict(
                {
                    **task.to_dict(),
                    ...
                    "resolution_note": str(payload.get("note") or ""),
                }
            ),
        ...
        self.project_store.append_event(
            project_id,
            "review_task_needs_more_work",
            {"task_id": task.task_id, "follow_up_task_id": follow_up.task_id, "version_id": task.applied_version_id},
        )
    ...
    def _send_nodes_list(self, job: JobState) -> None:
        records = _api_store_factories.node_store(_interfaces_api_runtime.Path(job.output_dir)).list_nodes()
        self._send_json(
            {
                "job_id": job.job_id,
    ...
    def _send_node_retry(self, method: str, job: JobState, tail: str) -> None:
        ...
        retried_job, status, error, retry = self.store.retry_job_node(job.job_id, node_name)
        if error is not None:
            self._send_error(status, error)
            return
        self._send_json(
            {"ok": True, "job": retried_job.to_dict() if retried_job is not None else None, "retry": retry},
            status=status,
        )
    ...
    def _send_node_route(self, method: str, job: JobState, tail: str) -> None:
        ...
        if len(parts) == 2:
            ...
            try:
                record = _api_store_factories.node_store(_interfaces_api_runtime.Path(job.output_dir)).read_node(_interfaces_api_runtime.unquote(node_name))
            ...
            self._send_json(
                {
                    "job_id": job.job_id,
                    "node": node_name,
                    "upstream": normalize_json_value(upstream),
                    "downstream": normalize_json_value(downstream),
