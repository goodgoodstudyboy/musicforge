    ...
    def _dispatch_ucc_evidence_detail(self, method, center_id, tail) -> bool:
        if tail.startswith('/evidence-reviews/'):
            ...
            if review_action == '/responses/import':
                ...
                self._send_json({'ok': response.get('status') == 'current', 'response': response, 'summary': {'response_id': response.get('response_id')}, 'status': response.get('status')}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return True
