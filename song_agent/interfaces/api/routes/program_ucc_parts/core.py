...
class ProgramUccCoreRoutes:
    def _dispatch_ucc_core(self, method, center_id, tail) -> bool:
        if tail in {'', '/'}:
            ...
            inventory = _interfaces_api_runtime.read_json(self.unified_command_center_store.inventory_path(center_id)) if self.unified_command_center_store.inventory_path(center_id).exists() else {}
            readiness = _interfaces_api_runtime.read_json(self.unified_command_center_store.readiness_path(center_id)) if self.unified_command_center_store.readiness_path(center_id).exists() else {}
            gap_plan = _interfaces_api_runtime.read_json(self.unified_command_center_store.gap_plan_path(center_id)) if self.unified_command_center_store.gap_plan_path(center_id).exists() else {}
            runbook = _interfaces_api_runtime.read_json(self.unified_command_center_store.runbook_path(center_id)) if self.unified_command_center_store.runbook_path(center_id).exists() else {}
        ...
        if tail == '/archive':
            if method == 'GET':
                manifest = _interfaces_api_runtime.read_json(self.unified_command_center_signoff_store.archive_manifest_path(center_id)) if self.unified_command_center_signoff_store.archive_manifest_path(center_id).exists() else {}
