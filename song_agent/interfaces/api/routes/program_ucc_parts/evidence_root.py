...
class ProgramUccEvidence_RootRoutes(ProgramUccRouteContext):
    def _dispatch_ucc_evidence_root(self, method, center_id, tail) -> bool:
        if tail == '/evidence-reviews':
            if method == 'GET':
                reviews = self.server.unified_command_center_evidence_review_store.list_reviews(center_id)
                self._send_json({'ok': True, 'reviews': reviews, 'summary': {'review_count': len(reviews)}})
                return True
            if method == 'POST':
                docs = self.server.unified_command_center_evidence_review_store.create_review(center_id, self._optional_json_body())
