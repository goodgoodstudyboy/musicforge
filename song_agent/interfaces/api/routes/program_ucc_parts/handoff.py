...
class ProgramUccHandoffRoutes:
    def _dispatch_ucc_handoff(self, method, center_id, tail) -> bool:
        ...
        if tail == '/handoff':
            if method == 'GET':
                manifest = _interfaces_api_runtime.read_json(self.unified_command_center_handoff_store.manifest_path(center_id)) if self.unified_command_center_handoff_store.manifest_path(center_id).exists() else {}
