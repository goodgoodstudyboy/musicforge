...
class TrustPortfolioAttestationRoutes:
    def _dispatch_portfolio_attestation(self, method, parts, portfolio_id, action) -> bool:
        if action == 'governance-attestation':
            ...
            if len(parts) == 2:
                ...
                self._send_json({'ok': True, 'portfolio_id': portfolio_id, 'profile': query_profile, 'report': report, 'certificate': certificate, 'verification': read_json(verification_path) if verification_path.exists() else {}, 'summary': summary, 'stale': stale})
                return True
            ...
            if subaction == 'verify' and len(parts) == 3:
                ...
                report = verify_release_portfolio_governance_attestation(self.release_portfolio_governance_attestation_store.zip_path(portfolio_id, profile), strict=bool(payload.get('strict', False)), require_vault=bool(payload.get('require_vault', False)), require_final_board=bool(payload.get('require_final_board', False)))
