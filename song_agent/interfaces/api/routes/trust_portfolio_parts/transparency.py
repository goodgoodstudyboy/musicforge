...
class TrustPortfolioTransparencyRoutes(TrustPortfolioRouteContext):
    def _dispatch_portfolio_transparency(self, method: str, parts: list[str], portfolio_id: str, action: str) -> bool:
        if action == 'governance-attestation-transparency':
            ...
            if len(parts) == 2:
                ...
                summary: JsonDocument = (
                    _interfaces_api_runtime.portfolio_governance_attestation_transparency_summary(feed)
                    if feed
                ...
                verification = _interfaces_api_runtime.read_json(verification_path) if verification_path.exists() else {}
            ...
            if subaction == 'refresh' and len(parts) == 3:
                ...
                feed = self.release_portfolio_governance_attestation_transparency_store.refresh_feed(portfolio_id, payload, now=_interfaces_api_runtime._utc_now())
                self._send_json({'ok': True, 'portfolio_id': portfolio_id, 'feed': feed, 'summary': _interfaces_api_runtime.portfolio_governance_attestation_transparency_summary(feed)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return True
            if subaction == 'export' and len(parts) == 3:
                ...
                payload = self._optional_json_body()
                payload.setdefault('profile', query_profile)
                manifest = self.release_portfolio_governance_attestation_transparency_store.export_transparency(portfolio_id, payload, now=_interfaces_api_runtime._utc_now())
                self._send_json({'ok': True, 'portfolio_id': portfolio_id, 'manifest': manifest, 'summary': manifest.get('current_public_state', {})}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return True
            if subaction == 'zip' and len(parts) == 3:
                ...
                payload = self._optional_json_body()
                payload.setdefault('profile', query_profile)
                zip_info = self.release_portfolio_governance_attestation_transparency_store.build_zip(portfolio_id, payload, now=_interfaces_api_runtime._utc_now())
            ...
            if subaction == 'verify' and len(parts) == 3:
                ...
                report = _interfaces_api_runtime.verify_release_portfolio_governance_attestation_transparency(self.release_portfolio_governance_attestation_transparency_store.zip_path(portfolio_id, profile), strict=bool(payload.get('strict', False)), require_current=bool(payload.get('require_current', False)), require_accepted_evidence=bool(payload.get('require_accepted_evidence', False)), require_no_revoked_current=bool(payload.get('require_no_revoked_current', False)), require_contiguous_chain=bool(payload.get('require_contiguous_chain', False)))
                _interfaces_api_runtime.write_release_portfolio_governance_attestation_transparency_verification_report(report, self.release_portfolio_governance_attestation_transparency_store.verification_report_path(portfolio_id, profile))
            ...
            if subaction == 'notices' and len(parts) == 4:
                ...
                return True
            self._send_error(_interfaces_api_runtime.HTTPStatus.NOT_FOUND, 'Release Portfolio Governance Attestation Transparency route not found.')
            return True
        return False
