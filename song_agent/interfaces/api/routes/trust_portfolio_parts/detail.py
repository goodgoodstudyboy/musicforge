...
class TrustPortfolioDetailRoutes(TrustPortfolioRouteContext):
    def _dispatch_portfolio_detail(self, method: str, parts: list[str], portfolio_id: str, action: str) -> bool:
        if len(parts) == 1:
            if method != 'GET':
            ...
            summary: JsonDocument = _interfaces_api_runtime.sanitize_metadata(
                _interfaces_api_runtime.portfolio_audit_summary(report) if report else {"status": "missing"}
            )
            summary['stale'] = stale
