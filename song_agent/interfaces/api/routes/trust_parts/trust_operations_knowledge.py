    ...
    def _handle_trust_operations_knowledge(self, method: str, hub_id: str, tail: str) -> None:
        try:
            ...
            if len(parts) >= 2 and parts[0] == "entries":
                entry_id = _interfaces_api_runtime.unquote(parts[1])
                if len(parts) == 2:
                    if method != "GET":
                        self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                        return
            ...
            if len(parts) >= 2 and parts[0] == "guards":
                guard_id = _interfaces_api_runtime.unquote(parts[1])
                if len(parts) == 2:
                    if method != "GET":
                        self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                        return
        ...
        try:
            ...
            if tail == "/exceptions":
                if method == "GET":
                    ...
                    return
                if method == "POST":
                    exception = self.trust_operations_control_signoff_store.request_exception(hub_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                    self._send_json({"ok": True, "hub_id": hub_id, "exception": exception}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            if tail == "/change-requests":
                if method == "GET":
                    ...
                    return
                if method == "POST":
                    cr = self.trust_operations_control_signoff_store.create_change_request(hub_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                    self._send_json({"ok": True, "hub_id": hub_id, "change_request": cr}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            if tail == "/reset":
            ...
            if len(parts) == 3 and parts[0] == "exceptions" and parts[2] in {"approve", "reject"}:
                if method != "POST":
                    ...
                    return
                if parts[2] == "approve":
                    exception = self.trust_operations_control_signoff_store.approve_exception(hub_id, _interfaces_api_runtime.unquote(parts[1]), self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                else:
                    exception = self.trust_operations_control_signoff_store.reject_exception(hub_id, _interfaces_api_runtime.unquote(parts[1]), self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
            ...
            if len(parts) == 3 and parts[0] == "change-requests" and parts[2] == "approve":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                cr = self.trust_operations_control_signoff_store.approve_change_request(hub_id, _interfaces_api_runtime.unquote(parts[1]), self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
