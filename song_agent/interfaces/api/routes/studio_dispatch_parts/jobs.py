    ...
    def _dispatch_studio_jobs(self, method, path, parsed) -> bool:
        ...
        if path == "/api/batches":
            if method == "GET":
                ...
                include_hidden = query.get("include_hidden", ["0"])[0] in {"1", "true", "yes"}
                self._send_json(
                    {"batches": [document.state.to_dict() for document in self.batch_store.list_batches(include_hidden=include_hidden)]}
                )
                return True
        ...
        if path == "/api/batches/import-csv":
            if method != "POST":
                ...
                return True
            payload = self._read_json_body()
            raw_max_concurrency = payload.get("max_concurrency", 1)
            max_concurrency = raw_max_concurrency if isinstance(raw_max_concurrency, (int, str)) else 1
