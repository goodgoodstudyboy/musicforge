        ...
        try:
                ...
                self._send_json(
                    {
                        ...
                        "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now()),
            ...
            if tail == "/analyze":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                analysis = self.mastering_store.analyze(release_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                ...
                self._send_json({"ok": True, "release_id": release_id, "analysis": analysis, "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now())})
                return
            if tail == "/plan":
                if method == "GET":
                    plan = self.mastering_store.read_plan(release_id, default={})
                    self._send_json({"ok": True, "release_id": release_id, "plan": plan, "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now())})
                    return
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                plan = self.mastering_store.build_plan(release_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self.release_store.append_event(release_id, "release_mastering_plan_created", {"action_count": plan.get("summary", {}).get("action_count")})
                self._send_json({"ok": True, "release_id": release_id, "plan": plan, "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now())})
                return
            if tail == "/candidates":
                if method == "GET":
                    self._send_json({"ok": True, "release_id": release_id, "candidates": self.mastering_store.list_candidates(release_id), "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now())})
                    return
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                candidate = self.mastering_store.render_candidate(release_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                ...
                self._send_json({"ok": True, "release_id": release_id, "candidate": candidate, "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now())}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
            if tail == "/refresh":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                result = self.mastering_store.refresh(release_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
            ...
            if tail == "/reset":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                result = self.mastering_store.reset(release_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
            ...
            if len(parts) >= 2 and parts[0] == "candidates":
                ...
                if len(parts) == 3 and parts[2] in {"review", "select"}:
                    if method != "POST":
                        ...
                        return
                    if parts[2] == "review":
                        candidate = self.mastering_store.review_candidate(release_id, candidate_id, self._read_json_body(), now=_interfaces_api_runtime._utc_now())
                    else:
                        candidate = self.mastering_store.select_candidate(release_id, candidate_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                    self._send_json({"ok": True, "release_id": release_id, "candidate": candidate, "summary": self.mastering_store.get_summary(release_id, now=_interfaces_api_runtime._utc_now())})
                    return
