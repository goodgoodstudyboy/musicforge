...
class QualityRoutesAcceptanceAnalyticsRefresh:
    def _handle_acceptance_analytics_refresh(self, method: str, query_string: str) -> None:
        ...
        try:
            ...
            scope = _interfaces_api_runtime.AnalyticsScope.from_values(
                ...
                suite_id=payload.get("suite_id") or _interfaces_api_runtime._query_value(query, "suite_id") or None,
                release_id=payload.get("release_id") or _interfaces_api_runtime._query_value(query, "release_id") or None,
                project_id=payload.get("project_id") or _interfaces_api_runtime._query_value(query, "project_id") or None,
            )
    ...
    def _handle_acceptance_fix_plans_root(self, method: str, query_string: str) -> None:
        try:
            if method == "GET":
                ...
                return
            if method == "POST":
                plan = self.acceptance_fix_plan_store.create(self._read_json_body(), now=_interfaces_api_runtime._utc_now())
    ...
    def _handle_acceptance_fix_plans_recommend(self, method: str) -> None:
        try:
            if method != "POST":
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            preview = self.acceptance_fix_plan_store.preview(self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
        ...
        try:
            ...
            if action == "refresh":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                review = self.acceptance_fix_plan_review_store.refresh_review(review_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "outcome_review": review.to_dict(), "summary": _interfaces_api_runtime.fix_plan_review_summary(review)})
                return
            if action == "archive":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                review = self.acceptance_fix_plan_review_store.archive_review(review_id, now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "outcome_review": review.to_dict(), "summary": _interfaces_api_runtime.fix_plan_review_summary(review)})
                return
    ...
    def _handle_planning_rulesets_root(self, method: str, query_string: str) -> None:
        try:
            if method == "GET":
                ...
                return
            if method == "POST":
                ruleset = self.planning_rule_simulation_store.create_ruleset(self._read_json_body(), now=_interfaces_api_runtime._utc_now())
        ...
        try:
            ...
            if action == "clone":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                ruleset = self.planning_rule_simulation_store.clone_ruleset(ruleset_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "ruleset": ruleset.to_dict(), "summary": _interfaces_api_runtime.ruleset_summary(ruleset)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
            if action == "archive":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                ruleset = self.planning_rule_simulation_store.archive_ruleset(ruleset_id, now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "ruleset": ruleset.to_dict(), "summary": _interfaces_api_runtime.ruleset_summary(ruleset)})
                return
            if action == "validate":
                if method != "POST":
    ...
    def _handle_planning_simulations_root(self, method: str, query_string: str) -> None:
        try:
            if method == "GET":
                ...
                return
            if method == "POST":
                simulation = self.planning_rule_simulation_store.create_simulation(self._read_json_body(), now=_interfaces_api_runtime._utc_now())
        ...
        try:
            ...
            if action == "refresh":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                simulation = self.planning_rule_simulation_store.refresh_simulation(simulation_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "simulation": simulation.to_dict(), "summary": _interfaces_api_runtime.planning_simulation_summary(simulation)})
                return
            if action == "archive":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                simulation = self.planning_rule_simulation_store.archive_simulation(simulation_id, now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "simulation": simulation.to_dict(), "summary": _interfaces_api_runtime.planning_simulation_summary(simulation)})
                return
    ...
    def _handle_planning_rule_governance_version_route(self, method: str, route: tuple[str, str]) -> None:
        ...
        try:
            ...
            active = self.planning_rule_governance_store.active_pointer()
            self._send_json({"ok": True, "version": version.to_dict(), "frozen_ruleset_summary": _interfaces_api_runtime.ruleset_summary(frozen), "summary": _interfaces_api_runtime.governance_summary(version, active=active, evidence_stale=self.planning_rule_governance_store.version_evidence_is_stale(version))})
    ...
    def _handle_planning_rule_governance_promotions(self, method: str, query_string: str) -> None:
        try:
            if method == "GET":
                ...
                return
            if method == "POST":
                promotion = self.planning_rule_governance_store.create_promotion(self._read_json_body(), now=_interfaces_api_runtime._utc_now())
