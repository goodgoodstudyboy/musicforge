...
class QualityRoutesPlanningRuleGovernancePromotion(QualityRouteContext):
        ...
        try:
            if not action:
                if method != "GET":
                    ...
                    return
                promotion = self.server.planning_rule_governance_store.read_promotion(promotion_id)
            ...
            if action == "approve":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                promotion = self.server.planning_rule_governance_store.approve_promotion(promotion_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "promotion": promotion.to_dict(), "summary": _interfaces_api_runtime.promotion_summary(promotion)})
                return
            if action == "reject":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                promotion = self.server.planning_rule_governance_store.reject_promotion(promotion_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "promotion": promotion.to_dict(), "summary": _interfaces_api_runtime.promotion_summary(promotion)})
                return
            if action == "promote":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                result = self.server.planning_rule_governance_store.promote(promotion_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
    ...
    def _handle_planning_rule_governance_rollback(self, method: str) -> None:
        if method != "POST":
            ...
            return
        try:
            result = self.server.planning_rule_governance_store.rollback(self._read_json_body(), now=_interfaces_api_runtime._utc_now())
    ...
    def _handle_planning_rule_governance_events(self, method: str, query_string: str) -> None:
        ...
        events = self.server.planning_rule_governance_store.events(limit=limit)
    ...
    def _handle_planning_rule_impact_reports(self, method: str, query_string: str) -> None:
        try:
            if method == "GET":
                ...
                reports = self.server.planning_rule_impact_store.list_reports(include_archived=include_archived, release_id=release_id, project_id=project_id)
                ...
                return
            if method == "POST":
                report = self.server.planning_rule_impact_store.refresh(self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
    ...
    def _handle_planning_rule_impact_latest(self, method: str, query_string: str) -> None:
        if method != "GET":
            ...
            return
        query = _interfaces_api_runtime.parse_qs(query_string)
        summary = self.server.planning_rule_impact_store.latest_summary(release_id=_interfaces_api_runtime._query_value(query, "release_id") or None, project_id=_interfaces_api_runtime._query_value(query, "project_id") or None)
        ...
        try:
            ...
            if action == "refresh":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                report = self.server.planning_rule_impact_store.refresh_report(report_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
            ...
            if action == "archive":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                report = self.server.planning_rule_impact_store.archive_report(report_id, now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "impact_report": report.to_dict(), "summary": _interfaces_api_runtime.planning_rule_impact_summary(report)})
                return
        ...
        try:
            ...
            if action == "refresh":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                plan = self.server.acceptance_fix_plan_store.refresh_plan(plan_id, now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "fix_plan": plan.to_dict(), "summary": _interfaces_api_runtime.fix_plan_summary(plan)})
                return
            if action == "archive":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                plan = self.server.acceptance_fix_plan_store.archive_plan(plan_id, now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "fix_plan": plan.to_dict(), "summary": _interfaces_api_runtime.fix_plan_summary(plan)})
                return
            if action == "create-fix-sprint":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                result = self.server.acceptance_fix_plan_store.create_fix_sprint(plan_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
            ...
            if action == "outcome-review":
                if method != "GET":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                review = self.server.acceptance_fix_plan_review_store.get_or_missing_for_plan(plan_id)
            ...
            if action == "outcome-review/refresh":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                outcome_review = self.server.acceptance_fix_plan_review_store.refresh_for_plan(plan_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "outcome_review": outcome_review.to_dict(), "summary": _interfaces_api_runtime.fix_plan_review_summary(outcome_review)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
