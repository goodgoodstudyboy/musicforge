        ...
        try:
            if tail in {"", "/"}:
                ...
                if method == "POST":
                    ...
                    self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "summary": _interfaces_api_runtime.submission_batch_summary(batch)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
            ...
            if action == "":
                if method == "GET":
                    ...
                    self._send_json({"ok": True, "release_id": release_id, "submission": self._submission_payload_with_evidence_summary(release_id, batch), "summary": _interfaces_api_runtime.submission_batch_summary(batch), "qa_summary": _interfaces_api_runtime.submission_qa_summary(qa), "signoff_summary": _interfaces_api_runtime.submission_signoff_summary(signoff), "events": self.submission_store.read_events(release_id, submission_id)})
                    return
                if method in {"POST", "PATCH"}:
                    batch = self.submission_store.update_submission(release_id, submission_id, self._optional_json_body())
                    self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "summary": _interfaces_api_runtime.submission_batch_summary(batch)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if action == "qa":
                ...
                self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "submission_qa": report, "summary": _interfaces_api_runtime.submission_qa_summary(report)})
                return
            ...
            if action == "export":
                if method == "GET":
                    try:
                        manifest = _interfaces_api_runtime.read_submission_export_manifest(self.submission_store, release_id, submission_id)
                    except FileNotFoundError:
                        self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "manifest": {}, "summary": _interfaces_api_runtime.submission_export_summary({})})
                        return
                    self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "manifest": manifest, "summary": _interfaces_api_runtime.submission_export_summary(manifest)})
                    return
                if method == "POST":
                    self.submission_store.ensure_mutable(batch)
                    report = self._get_or_refresh_submission_qa(release_id, batch, refresh=False)
                    manifest = _interfaces_api_runtime.build_submission_export_bundle(store=self.submission_store, release_id=release_id, submission=batch, qa_report=report, now=_interfaces_api_runtime._utc_now())
                    batch = self.submission_store.get_submission(release_id, submission_id)
                    self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "manifest": manifest, "summary": _interfaces_api_runtime.submission_export_summary(manifest)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if action == "export-zip":
                if method != "POST":
                    ...
                    return
                self.submission_store.ensure_mutable(batch)
                zip_info = _interfaces_api_runtime.build_submission_package_zip(self.submission_store, release_id, batch, now=_interfaces_api_runtime._utc_now())
                manifest = _interfaces_api_runtime.read_submission_export_manifest(self.submission_store, release_id, submission_id)
                batch = self.submission_store.update_export_summary(release_id, submission_id, _interfaces_api_runtime.submission_export_summary(manifest))
                self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "zip": zip_info, "summary": _interfaces_api_runtime.submission_export_summary(manifest)})
                return
            ...
            if action == "signoff":
                if method == "GET":
                    signoff = self.submission_store.read_signoff(release_id, submission_id, default={})
                    self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "signoff": signoff, "summary": _interfaces_api_runtime.submission_signoff_summary(signoff)})
                    return
                if method == "POST":
                    self.submission_store.ensure_mutable(batch)
                    report = self._get_or_refresh_submission_qa(release_id, batch, refresh=True)
                    ...
                    if bool(payload.get("require_rights_clearance", False)):
                        ...
                        if rights_gate.get("hard_block") and rights_gate.get("status") == "failed":
                            ...
                            return
                        try:
                            export_manifest = _interfaces_api_runtime.read_submission_export_manifest(self.submission_store, release_id, submission_id)
                        except FileNotFoundError:
                            export_manifest = {}
                    ...
                    signoff = _interfaces_api_runtime.sign_submission_package(store=self.submission_store, release_id=release_id, submission=batch, qa_report=report, payload=payload, now=_interfaces_api_runtime._utc_now())
                    batch = self.submission_store.get_submission(release_id, submission_id)
                    self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "signoff": signoff, "summary": _interfaces_api_runtime.submission_signoff_summary(signoff)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if action == "verify":
                if method != "POST":
                    ...
                    return
                payload = self._optional_json_body()
                report = _interfaces_api_runtime.verify_submission_package(
                ...
                _interfaces_api_runtime.write_submission_verification_report(report, self.submission_store.submission_dir(release_id, submission_id) / "submission-verification-report.json")
                self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "verification": report, "summary": _interfaces_api_runtime.submission_verification_summary(report)})
                return
            ...
            if action == "evidence-report-refresh":
                ...
                self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "evidence_report": report, "summary": _interfaces_api_runtime.submission_evidence_report_summary(report)})
                return
            ...
            if action == "evidence-export":
                if method == "GET":
                    ...
                    return
                if method == "POST":
                    manifest = self.submission_evidence_store.export_evidence(release_id, submission_id, now=_interfaces_api_runtime._utc_now())
                    self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "manifest": manifest, "summary": manifest.get("summary", {})}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if action == "evidence-export-zip":
                if method != "POST":
                    self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                    return
                zip_info = self.submission_evidence_store.build_zip(release_id, submission_id, now=_interfaces_api_runtime._utc_now())
            ...
            if action == "evidence-signoff":
                if method == "GET":
                    ...
                    self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "signoff": signoff, "summary": _interfaces_api_runtime.submission_evidence_signoff_summary(signoff)})
                    return
                if method == "POST":
                    signoff = self.submission_evidence_store.signoff_evidence(release_id, submission_id, self._optional_json_body(), now=_interfaces_api_runtime._utc_now())
                    self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "signoff": signoff, "summary": _interfaces_api_runtime.submission_evidence_signoff_summary(signoff)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if action == "evidence-verify":
                if method != "POST":
                    ...
                    return
                payload = self._optional_json_body()
                report = _interfaces_api_runtime.verify_submission_evidence_package(
                ...
                _interfaces_api_runtime.write_submission_evidence_verification_report(report, self.submission_store.submission_dir(release_id, submission_id) / "submission-evidence-verification-report.json")
                self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "verification": report, "summary": _interfaces_api_runtime.submission_evidence_verification_summary(report)})
                return
            ...
            if action == "evidence-upload-attachment":
                ...
                self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "item_id": item_id, "attachment": attachment}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
            ...
            if action == "evidence-submission-receipt":
                ...
                self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "evidence": evidence, "summary": _interfaces_api_runtime.submission_batch_summary(batch)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
            ...
            if action == "evidence-resubmission-round":
                ...
                self._send_json({"ok": True, "release_id": release_id, "submission_id": submission_id, "item_id": item_id, "round": round_record}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                return
            ...
            if action == "record-submission":
                if method != "POST":
                    ...
                    return
                batch, evidence = self.submission_evidence_store.record_submission(release_id, submission_id, item_id or "", self._optional_json_body())
                self._send_json({"ok": True, "release_id": release_id, "submission": batch.to_dict(), "evidence": evidence, "summary": _interfaces_api_runtime.submission_batch_summary(batch)})
                return
