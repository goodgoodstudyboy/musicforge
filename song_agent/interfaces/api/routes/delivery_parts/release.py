        ...
        try:
            if tail == "":
                if method == "GET":
                    ...
                    self._send_json({"ok": True, "release": document.to_dict(), "summary": _interfaces_api_runtime.release_summary(document), "events": self.release_store.read_events(release_id)})
                    return
                if method == "PATCH":
            ...
            if tail == "/qa":
                ...
                self._send_json({"ok": True, "release_id": release_id, "release_qa": report, "summary": _interfaces_api_runtime.release_qa_summary(report)})
                return
                    ...
                    self._send_json(
                        {
                            ...
                            "metadata": metadata,
                            "history": _interfaces_api_runtime.read_release_metadata_history(self.release_store, release_id),
                            "summary": _interfaces_api_runtime.release_metadata_summary(metadata, qa_report, _interfaces_api_runtime.metadata_export_summary(_interfaces_api_runtime._safe_read_release_export_manifest(self.release_store, release_id))),
                        }
                    )
                    return
                if method == "POST":
                    ...
                    self._send_json({"ok": True, "release_id": release_id, "metadata": metadata, "summary": _interfaces_api_runtime.release_metadata_summary(metadata, report)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if tail == "/metadata/init":
                ...
                metadata = _interfaces_api_runtime.initialize_release_metadata(self.release_store, release_id, force=bool(payload.get("force", False)), merge=bool(payload.get("merge", False)), now=_interfaces_api_runtime._utc_now())
                report = self._get_or_refresh_release_metadata_qa(release_id, refresh=True)
                self._send_json({"ok": True, "release_id": release_id, "metadata": metadata, "summary": _interfaces_api_runtime.release_metadata_summary(metadata, report)})
                return
            ...
            if tail == "/metadata/qa":
                ...
                self._send_json({"ok": True, "release_id": release_id, "metadata_qa": report, "summary": _interfaces_api_runtime.release_metadata_qa_summary(report)})
                return
            ...
            if tail == "/metadata/export":
                if method == "GET":
                    manifest = _interfaces_api_runtime._safe_read_release_export_manifest(self.release_store, release_id)
                    self._send_json({"ok": True, "release_id": release_id, "metadata_export": manifest.get("metadata", {}), "summary": _interfaces_api_runtime.metadata_export_summary(manifest)})
                    return
                if method == "POST":
                    self._ensure_release_export_mutable(release_id)
                    report = self._get_or_refresh_release_metadata_qa(release_id, refresh=False)
                    export_summary = _interfaces_api_runtime.export_release_metadata_files(release_store=self.release_store, release_id=release_id, qa_report=report, now=_interfaces_api_runtime._utc_now())
                    manifest = _interfaces_api_runtime.attach_metadata_export_to_manifest(self.release_store, release_id, export_summary)
                    _interfaces_api_runtime.build_release_export_zip(self.release_store, release_id, now=_interfaces_api_runtime._utc_now())
                    manifest = _interfaces_api_runtime.read_release_export_manifest(self.release_store, release_id)
                    document = self.release_store.update_export_summary(release_id, _interfaces_api_runtime.release_export_summary(manifest))
                    self.release_store.append_event(release_id, "release_metadata_exported", {"file_count": len(export_summary.get("files", []))})
                    self._send_json({"ok": True, "release": document.to_dict(), "manifest": manifest, "metadata_export": export_summary, "summary": _interfaces_api_runtime.metadata_export_summary(manifest)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if tail == "/export":
                if method == "GET":
                    try:
                        ...
                        return
                    self._send_json({"ok": True, "release_id": release_id, "manifest": manifest, "summary": _interfaces_api_runtime.release_export_summary(manifest)})
                    return
                if method == "POST":
                    document = self.release_store.get_release(release_id)
                    self._ensure_release_export_mutable(release_id, document=document)
                    report = self._get_or_refresh_release_qa(release_id, refresh=False, options={})
                    manifest = _interfaces_api_runtime.build_release_export_bundle(release=document, release_store=self.release_store, project_store=self.project_store, qa_report=report, now=_interfaces_api_runtime._utc_now())
                    document = self.release_store.update_export_summary(release_id, _interfaces_api_runtime.release_export_summary(manifest))
                    self.release_store.append_event(release_id, "release_export_created", {"file_count": manifest.get("summary", {}).get("file_count")})
                    self._send_json({"ok": True, "release": document.to_dict(), "manifest": manifest, "summary": _interfaces_api_runtime.release_export_summary(manifest)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            if tail == "/export/zip":
                ...
                self._send_json({"ok": True, "release": document.to_dict(), "zip": zip_info, "summary": _interfaces_api_runtime.release_export_summary(manifest)})
                return
