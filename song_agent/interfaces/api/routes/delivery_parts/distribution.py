        ...
        try:
            ...
            if tail == "/targets":
                ...
                if method == "POST":
                    ...
                    self._send_json({"ok": True, "release_id": release_id, "target": target.to_dict(), "summary": _interfaces_api_runtime.distribution_target_summary(target)}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            ...
            target_route = _interfaces_api_runtime._match_distribution_target_tail(tail)
            ...
            if action == "":
                if method == "GET":
                    ...
                    checklist = _interfaces_api_runtime.reconcile_distribution_checklist(self.distribution_store, release_id, target, template, write=False) if template else _interfaces_api_runtime.read_distribution_checklist(self.distribution_store, release_id, target_id, default={})
                    self._send_json({"ok": True, "release_id": release_id, "target": target.to_dict(), "template": template, "template_summary": _interfaces_api_runtime.template_summary(template) if template else {}, "checklist": checklist, "checklist_summary": _interfaces_api_runtime.checklist_summary(checklist), "summary": _interfaces_api_runtime.distribution_target_summary(target), "qa_summary": _interfaces_api_runtime.distribution_qa_summary(qa), "signoff_summary": _interfaces_api_runtime.distribution_signoff_summary(signoff)})
                    return
                if method in {"POST", "PATCH"}:
                    target = self.distribution_store.update_target(release_id, target_id, self._optional_json_body())
                    self._send_json({"ok": True, "release_id": release_id, "target": target.to_dict(), "summary": _interfaces_api_runtime.distribution_target_summary(target)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            if action == "delete":
            ...
            if action == "qa":
                ...
                self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "distribution_qa": report, "summary": _interfaces_api_runtime.distribution_qa_summary(report)})
                return
            if action == "checklist":
                template = self.distribution_store.resolve_target_template(target)
                if method == "GET":
                    checklist = _interfaces_api_runtime.reconcile_distribution_checklist(self.distribution_store, release_id, target, template, write=False) if template else _interfaces_api_runtime.read_distribution_checklist(self.distribution_store, release_id, target_id, default={})
                    self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "checklist": checklist, "summary": _interfaces_api_runtime.checklist_summary(checklist)})
                    return
                if method == "POST":
                    if not template:
                        self._send_error(_interfaces_api_runtime.HTTPStatus.BAD_REQUEST, "Distribution target has no template_pack_id.")
                        return
                    checklist = _interfaces_api_runtime.initialize_distribution_checklist(self.distribution_store, release_id, target, template, now=_interfaces_api_runtime._utc_now())
                    self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "checklist": checklist, "summary": _interfaces_api_runtime.checklist_summary(checklist)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            if action == "layout":
                ...
                self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "layout": layout, "summary": _interfaces_api_runtime.layout_summary(layout)})
                return
            if action.startswith("checklist-item:"):
                ...
                checklist = _interfaces_api_runtime.update_distribution_checklist_item(self.distribution_store, release_id, target, template, item_id, self._read_json_body(), now=_interfaces_api_runtime._utc_now())
                self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "checklist": checklist, "summary": _interfaces_api_runtime.checklist_summary(checklist)})
                return
            ...
            if action == "export":
                if method == "GET":
                    ...
                    if not package_id:
                        self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "manifest": {}, "summary": _interfaces_api_runtime.distribution_export_summary({})})
                        return
                    manifest = _interfaces_api_runtime.read_distribution_export_manifest(self.distribution_store, release_id, package_id)
                    self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "manifest": manifest, "summary": _interfaces_api_runtime.distribution_export_summary(manifest)})
                    return
                if method == "POST":
                    self.distribution_store.ensure_target_mutable(release_id, target)
                    report = self._get_or_refresh_distribution_qa(release_id, target, refresh=False)
                    manifest = _interfaces_api_runtime.build_distribution_export_package(store=self.distribution_store, release_id=release_id, target=target, qa_report=report, now=_interfaces_api_runtime._utc_now())
                    target = self.distribution_store.get_target(release_id, target_id)
                    self._send_json({"ok": True, "release_id": release_id, "target": target.to_dict(), "manifest": manifest, "summary": _interfaces_api_runtime.distribution_export_summary(manifest), "layout_summary": _interfaces_api_runtime.layout_summary(manifest.get("layout") if isinstance(manifest.get("layout"), dict) else {})}, status=_interfaces_api_runtime.HTTPStatus.CREATED)
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
            if action == "export-zip":
                if method != "POST":
                    ...
                    return
                self.distribution_store.ensure_target_mutable(release_id, target)
                zip_info = _interfaces_api_runtime.build_distribution_package_zip(self.distribution_store, release_id, target, now=_interfaces_api_runtime._utc_now())
                target = self.distribution_store.get_target(release_id, target_id)
                package_id = self.distribution_store.latest_package_id(target)
                manifest = _interfaces_api_runtime.read_distribution_export_manifest(self.distribution_store, release_id, package_id) if package_id else {}
                self._send_json({"ok": True, "release_id": release_id, "target": target.to_dict(), "zip": zip_info, "summary": _interfaces_api_runtime.distribution_export_summary(manifest)})
                return
            ...
            if action == "verify":
                ...
                report = _interfaces_api_runtime.verify_distribution_package(
                ...
                _interfaces_api_runtime.write_distribution_verification_report(report, self.distribution_store.package_dir(release_id, package_id) / "verification-report.json")
                self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "verification": report, "summary": _interfaces_api_runtime.distribution_verification_summary(report)})
                return
            if action == "signoff":
                if method == "GET":
                    signoff = self.distribution_store.read_signoff(release_id, target, default={})
                    self._send_json({"ok": True, "release_id": release_id, "target_id": target_id, "signoff": signoff, "summary": _interfaces_api_runtime.distribution_signoff_summary(signoff)})
                    return
                if method == "POST":
                    self.distribution_store.ensure_target_mutable(release_id, target)
                    report = self._get_or_refresh_distribution_qa(release_id, target, refresh=True)
                    ...
                    if require_encoded_review:
                        ...
                        required_profiles = [
                            profile_id
                            for profile_id in _interfaces_api_runtime.resolve_target_audio_format_profiles(target, template)
                        ...
                        if encoded_acceptance_gate.get("hard_block") and encoded_acceptance_gate.get("status") == "failed":
                            ...
                            return
                        package_id = self.distribution_store.latest_package_id(target)
                        export_manifest = _interfaces_api_runtime.read_distribution_export_manifest(self.distribution_store, release_id, package_id) if package_id else {}
                    ...
                    signoff = _interfaces_api_runtime.sign_distribution_package(store=self.distribution_store, release_id=release_id, target=target, qa_report=report, payload=payload, now=_interfaces_api_runtime._utc_now())
                    target = self.distribution_store.get_target(release_id, target_id)
                    self._send_json({"ok": True, "release_id": release_id, "target": target.to_dict(), "signoff": signoff, "summary": _interfaces_api_runtime.distribution_signoff_summary(signoff)})
                    return
                self._send_error(_interfaces_api_runtime.HTTPStatus.METHOD_NOT_ALLOWED, "Method not allowed.")
                return
