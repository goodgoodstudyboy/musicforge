...
def print_planning_ruleset_result(result: JsonDocument) -> None:
    ...
    if result.get("rulesets") is not None:
        print(f"rulesets: {_array_length(result.get('rulesets'))}")

def print_planning_simulation_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    ...
    if result.get("simulations") is not None:
        print(f"simulations: {_array_length(result.get('simulations'))}")

def print_planning_rule_governance_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    ...
    if promotion:
        ...
        print(f"recommendation: {_as_document(promotion.get('evidence')).get('recommendation')}")
    if result.get("versions") is not None:
        print(f"versions: {_array_length(result.get('versions'))}")
    if result.get("promotions") is not None:
        print(f"promotions: {_array_length(result.get('promotions'))}")
    if result.get("events") is not None:
        print(f"events: {_array_length(result.get('events'))}")

def print_planning_rule_impact_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    ...
    if result.get("reports") is not None:
        print(f"reports: {_array_length(result.get('reports'))}")

def print_acceptance_kb_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    ...
    if result.get("entries") is not None:
        print(f"listed_entries: {_array_length(result.get('entries'))}")
    if recommendation:
...
def _execute_verify_release_audio_baseline_registry_package(argv: list[str]) -> None:
    ...
    if args.json:
        ...
        for check in _as_documents(report.get("checks")):
