...
def _execute_release_audio_review(argv: list[str]) -> None:
    ...
    project_store = _quality_stores.project_store()
    release_store = _quality_stores.release_store(project_store=project_store)
    store = _quality_stores.audio_review_evidence_store(release_store, project_store)
    if args.action == "list":
...
def _execute_encoded_audio_acceptance(argv: list[str]) -> None:
    ...
    project_store = _quality_stores.project_store()
    release_store = _quality_stores.release_store(project_store=project_store)
    profile_store = _quality_stores.audio_encoding_profile_store(release_store.root.parent / "audio-encoding-profiles")
    encoding_store = _quality_stores.audio_encoding_store(release_store, project_store=project_store, profile_store=profile_store)
    store = _quality_stores.encoded_audio_acceptance_store(release_store, project_store=project_store, audio_encoding_store=encoding_store)
...
def _execute_format_decision(argv: list[str]) -> None:
    ...
    profile_store = _quality_stores.audio_encoding_profile_store(release_store.root.parent / "audio-encoding-profiles")
    encoding_store = _quality_stores.audio_encoding_store(release_store, project_store=project_store, profile_store=profile_store)
    distribution_store = _quality_stores.distribution_store(release_store)
    store = _quality_stores.format_decision_store(release_store, project_store=project_store, encoding_store=encoding_store, distribution_store=distribution_store)
...
def _execute_acceptance_analytics(argv: list[str]) -> None:
    ...
    store = _quality_stores.acceptance_analytics_store()
...
def _execute_acceptance_fix_sprint(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    store = _quality_stores.acceptance_fix_sprint_store()
    ...
    if args.json:
        ...
        print_acceptance_fix_sprint_result(normalize_json_document(result))
    raise SystemExit(0)
...
def _execute_acceptance_fix_plan(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    store = _quality_stores.acceptance_fix_planning_store()
    if args.action == "create":
        ...
        review_store = _quality_stores.acceptance_fix_plan_review_store(plan_store=store, fix_sprint_store=store.fix_sprint_store, kb_store=store.kb_store, project_store=store.project_store)
        if args.refresh:
    ...
    if args.json:
        ...
        print_acceptance_fix_plan_result(normalize_json_document(result))
    raise SystemExit(0)
...
def _execute_planning_ruleset(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    store = _quality_stores.planning_rule_simulation_store()
    if args.action == "create":
    ...
    if args.json:
        ...
        print_planning_ruleset_result(normalize_json_document(result))
    raise SystemExit(0)
...
def _execute_planning_simulation(argv: list[str]) -> None:
    ...
    if args.json:
        ...
        print_planning_simulation_result(normalize_json_document(result))
    raise SystemExit(0)
