...
def _execute_trust_operations_final_readiness(argv: list[str]) -> None:
    ...
    store = _trust_stores.trust_operations_final_readiness_store()
    result: JsonDocument = {"ok": True}
    source_payload = _trust_operations_final_readiness_source_payload(args)
    if args.refresh_report:
        result.update(store.refresh_report(dict(source_payload)))
    ...
    if args.export:
        result["manifest"] = store.export_handoff(dict(source_payload))
    if args.zip:
    ...
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_final_handoff_verification_report(_as_document(result.get("verification")))
        else:
...
def _execute_trust_operations_controls(argv: list[str]) -> None:
    ...
    incident_store = _trust_stores.trust_operations_incident_store(hub_store=hub_store)
    knowledge_store = _trust_stores.trust_operations_incident_knowledge_store(hub_store=hub_store, incident_store=incident_store)
    store = _trust_stores.trust_operations_control_store(hub_store=hub_store, incident_store=incident_store, knowledge_store=knowledge_store)
    result: JsonDocument = {"ok": True, "hub_id": args.hub_id}
    source_payload = {
    ...
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_control_verification_report(_as_document(result.get("verification")))
        else:
...
def _execute_trust_operations_assurance(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    hub_store = _trust_stores.trust_operations_hub_store()
    store = _trust_stores.trust_operations_assurance_store(hub_store=hub_store)
    result: JsonDocument = {"ok": True, "hub_id": args.hub_id}
    source_payload = _trust_operations_assurance_source_payload(args)
    if args.list:
        result["runs"] = normalize_json_value(store.list_runs(args.hub_id))
    if args.refresh:
    ...
    if args.export:
        ...
        result["manifest"] = store.export_archive(args.run_id, dict(source_payload))
    if args.zip:
        if not args.run_id:
            raise ValueError("--run-id is required for --zip unless --refresh was used.")
        result["zip"] = store.build_archive_zip(args.run_id, dict(source_payload))
    if args.verify:
        if not args.run_id:
    ...
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_assurance_verification_report(_as_document(result.get("verification")))
        else:
...
def _execute_trust_operations_control_signoff(argv: list[str]) -> None:
    ...
    incident_store = _trust_stores.trust_operations_incident_store(hub_store=hub_store)
    knowledge_store = _trust_stores.trust_operations_incident_knowledge_store(hub_store=hub_store, incident_store=incident_store)
    control_store = _trust_stores.trust_operations_control_store(hub_store=hub_store, incident_store=incident_store, knowledge_store=knowledge_store)
    store = _trust_stores.trust_operations_control_signoff_store(control_store=control_store, hub_store=hub_store, incident_store=incident_store, knowledge_store=knowledge_store)
    result: JsonDocument = {"ok": True, "hub_id": args.hub_id}
    source_payload = {
    ...
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_control_signoff_verification_report(_as_document(result.get("verification")))
        else:
...
def _execute_trust_operations_hub_runbook(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    hub_store = _trust_stores.trust_operations_hub_store()
    store = _trust_stores.trust_operations_hub_runbook_store(hub_store=hub_store)
    result: JsonDocument = {"ok": True, "hub_id": args.hub_id}
    ...
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_hub_runbook_verification_report(_as_document(result.get("verification")))
        else:
