...
class _TrustOperationsAssuranceSourcePayload(TypedDict):
    hub_package_path: _Path | str | None
    hub_verification_report_path: _Path | str | None
    control_signoff_archive_path: _Path | str | None
    control_signoff_verification_report_path: _Path | str | None
    control_package_path: _Path | str | None
    control_verification_report_path: _Path | str | None
    incident_board_package_path: _Path | str | None
    incident_board_verification_report_path: _Path | str | None
    incident_knowledge_package_path: _Path | str | None
    incident_knowledge_verification_report_path: _Path | str | None
    release_verification_paths: list[_Path | str]
    distribution_verification_paths: list[_Path | str]
    submission_verification_paths: list[_Path | str]
    submission_evidence_verification_paths: list[_Path | str]
    release_operations_verification_paths: list[_Path | str]


class _TrustOperationsAssuranceWatchSourcePayload(TypedDict):
    assurance_archive_path: _Path | str | None
    assurance_verification_report_path: _Path | str | None
    hub_package_path: _Path | str | None
    hub_verification_report_path: _Path | str | None


class _TrustOperationsFinalReadinessSourcePayload(_TrustOperationsAssuranceSourcePayload):
    continuous_assurance_archive_path: _Path | str | None
    continuous_assurance_verification_report_path: _Path | str | None
    assurance_watch_package_path: _Path | str | None
    assurance_watch_verification_report_path: _Path | str | None
    assurance_watch_signoff_archive_path: _Path | str | None
    assurance_watch_signoff_verification_report_path: _Path | str | None


def _trust_operations_assurance_source_payload(args: argparse.Namespace) -> _TrustOperationsAssuranceSourcePayload:
    return {
...
def _trust_operations_assurance_watch_source_payload(args: argparse.Namespace) -> _TrustOperationsAssuranceWatchSourcePayload:
    return {
...
def _trust_operations_final_readiness_source_payload(args: argparse.Namespace) -> _TrustOperationsFinalReadinessSourcePayload:
    return {
...
def print_release_portfolio_audit_result(result: JsonDocument) -> None:
    ...
    if result.get("portfolios") is not None:
        print(f"portfolios: {len(_as_list(result.get('portfolios')))}")
    if result.get("zip"):
        print(f"zip: {_as_document(result.get('zip')).get('filename')}")
    if verification:
        print(f"verify: {verification.get('status')}")

def print_release_portfolio_governance_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    verification = _as_document(result.get("verification_summary"))
    ...
    if result.get("queues") is not None:
        print(f"queues: {len(_as_list(result.get('queues')))}")
    if result.get("zip"):
        print(f"zip: {_as_document(result.get('zip')).get('filename')}")
    if verification:
        print(f"verify: {verification.get('status')}")
