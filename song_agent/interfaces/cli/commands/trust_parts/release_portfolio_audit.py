...
def _execute_release_portfolio_audit(argv: list[str]) -> None:
    ...
    signoff_store = _trust_stores.release_operations_signoff_store(operations_store=operations_store, runbook_store=runbook_store, release_store=release_store)
    audit_store = _trust_stores.release_operations_audit_store(operations_store=operations_store, runbook_store=runbook_store, signoff_store=signoff_store, release_store=release_store)
    reviewer_store = _trust_stores.release_operations_reviewer_pack_store(audit_store=audit_store, signoff_store=signoff_store, release_store=release_store)
    store = _trust_stores.release_portfolio_audit_store(release_store=release_store, operations_store=operations_store, runbook_store=runbook_store, signoff_store=signoff_store, audit_store=audit_store, reviewer_pack_store=reviewer_store)
    result: JsonDocument = {"ok": True}
    ...
    if args.list:
        ...
        result.update({"portfolios": normalize_json_value(portfolios), "summary": {"count": len(portfolios)}})
    else:
        if args.create:
...
def _execute_release_portfolio_governance_queue(argv: list[str]) -> None:
    ...
    audit_store = _trust_stores.release_operations_audit_store(operations_store=operations_store, runbook_store=runbook_store, signoff_store=signoff_store, release_store=release_store)
    reviewer_store = _trust_stores.release_operations_reviewer_pack_store(audit_store=audit_store, signoff_store=signoff_store, release_store=release_store)
    portfolio_store = _trust_stores.release_portfolio_audit_store(release_store=release_store, operations_store=operations_store, runbook_store=runbook_store, signoff_store=signoff_store, audit_store=audit_store, reviewer_pack_store=reviewer_store)
    store = _trust_stores.release_portfolio_governance_store(portfolio_store=portfolio_store, reviewer_pack_store=reviewer_store, audit_store=audit_store, signoff_store=signoff_store)
    result: JsonDocument = {"ok": True}
    if args.list:
        queues = store.list_queues(portfolio_id=args.portfolio_id or None, include_archived=True)
        result.update({"queues": normalize_json_value(queues), "summary": {"count": len(queues)}})
    else:
        if args.create:
            if not args.portfolio_id:
...
def _execute_release_portfolio_governance_signoff(argv: list[str]) -> None:
    ...
    audit_store = _trust_stores.release_operations_audit_store(operations_store=operations_store, runbook_store=runbook_store, signoff_store=operations_signoff_store, release_store=release_store)
    reviewer_store = _trust_stores.release_operations_reviewer_pack_store(audit_store=audit_store, signoff_store=operations_signoff_store, release_store=release_store)
    portfolio_store = _trust_stores.release_portfolio_audit_store(release_store=release_store, operations_store=operations_store, runbook_store=runbook_store, signoff_store=operations_signoff_store, audit_store=audit_store, reviewer_pack_store=reviewer_store)
    governance_store = _trust_stores.release_portfolio_governance_store(portfolio_store=portfolio_store, reviewer_pack_store=reviewer_store, audit_store=audit_store, signoff_store=operations_signoff_store)
    store = _trust_stores.release_portfolio_governance_signoff_store(governance_store=governance_store)
    queue_id = args.queue_id
    result: JsonDocument = {"ok": True, "queue_id": queue_id}
...
def _execute_release_portfolio_governance_audit(argv: list[str]) -> None:
    ...
    reviewer_store = _trust_stores.release_operations_reviewer_pack_store(audit_store=operations_audit_store, signoff_store=operations_signoff_store, release_store=release_store)
    portfolio_store = _trust_stores.release_portfolio_audit_store(release_store=release_store, operations_store=operations_store, runbook_store=runbook_store, signoff_store=operations_signoff_store, audit_store=operations_audit_store, reviewer_pack_store=reviewer_store)
    governance_store = _trust_stores.release_portfolio_governance_store(portfolio_store=portfolio_store, reviewer_pack_store=reviewer_store, audit_store=operations_audit_store, signoff_store=operations_signoff_store)
    signoff_store = _trust_stores.release_portfolio_governance_signoff_store(governance_store=governance_store)
    store = _trust_stores.release_portfolio_governance_audit_store(portfolio_store=portfolio_store, governance_store=governance_store, signoff_store=signoff_store)
    portfolio_id = args.portfolio_id
    result: JsonDocument = {"ok": True, "portfolio_id": portfolio_id}
    if args.refresh:
        ...
        report = store.read_report(portfolio_id, default={})
        summary: JsonDocument = portfolio_governance_audit_summary(report) if report else {"status": "missing"}
        if report:
            summary["stale"] = store.report_is_stale(portfolio_id, report)
        result.update({"report": report, "summary": summary, "stale": summary.get("stale", False)})
    if args.ledger:
        ...
        result.update({"ledger": normalize_json_value(entries), "ledger_summary": {"entry_count": len(entries)}})
    if args.export:
