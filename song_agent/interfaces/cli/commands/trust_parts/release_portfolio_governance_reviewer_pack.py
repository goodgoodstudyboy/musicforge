...
def _execute_release_portfolio_governance_reviewer_pack(argv: list[str]) -> None:
    ...
    audit_store = _trust_stores.release_portfolio_governance_audit_store(portfolio_store=portfolio_store, governance_store=governance_store, signoff_store=signoff_store)
    store = _trust_stores.release_portfolio_governance_reviewer_pack_store(audit_store=audit_store)
    ...
    if args.refresh:
        ...
        summary: JsonDocument = portfolio_governance_reviewer_pack_summary(report) if report else {"status": "missing"}
        if report:
...
def _execute_release_portfolio_governance_final_board(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_final_board_store(portfolio_store=portfolio_store, audit_store=governance_audit_store, reviewer_pack_store=governance_reviewer_store)
    portfolio_id = args.portfolio_id
    result: JsonDocument = {"ok": True, "portfolio_id": portfolio_id}
    ...
    if args.refresh or args.import_reviewer_response is not None:
        ...
        report = store.read_report(portfolio_id, default={})
        summary: JsonDocument = portfolio_governance_final_board_summary(report) if report else {"status": "missing"}
        if report:
            summary["stale"] = store.report_is_stale(portfolio_id, report)
        result.update({"report": report, "summary": summary, "stale": summary.get("stale", False)})
...
def _execute_release_portfolio_governance_evidence_vault(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_evidence_vault_store(
    ...
    if args.refresh:
        ...
        report = store.read_report(portfolio_id, default={})
        summary: JsonDocument = portfolio_governance_evidence_vault_summary(report) if report else {"status": "missing"}
        if report:
            summary["stale"] = store.report_is_stale(portfolio_id, report)
        result.update({"report": report, "summary": summary, "stale": summary.get("stale", False)})
