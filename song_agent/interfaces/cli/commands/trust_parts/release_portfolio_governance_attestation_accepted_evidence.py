...
def _execute_release_portfolio_governance_attestation_accepted_evidence(argv: list[str]) -> None:
    ...
    from song_agent.interfaces.bootstrap.cli.trust import portfolio_accepted_evidence_summary
    pass
    pass
    ...
    store = _trust_stores.release_portfolio_governance_attestation_accepted_evidence_store(review_store=review_store)
    ...
    if args.refresh:
        ...
        result.update({"accepted_evidence": evidence, "summary": portfolio_accepted_evidence_summary(evidence), "stale": False})
    else:
        evidence = store.read_evidence(portfolio_id, profile=args.profile, default={})
        summary = portfolio_accepted_evidence_summary(evidence) if evidence else {"status": "missing", "external_review_status": "missing", "profile": args.profile}
        if evidence:
    ...
    if args.archive:
        ...
        result["summary"] = portfolio_accepted_evidence_summary(_as_document(result.get("accepted_evidence")))
...
def _execute_release_portfolio_governance_attestation_transparency(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_attestation_transparency_store(
    ...
    if args.verify:
        ...
        result["verification"] = verification
    if args.notices:
        result["notices"] = normalize_json_value(store.list_notices(portfolio_id, profile=args.profile))
    if args.report_out is not None:
        write_interface_document(args.report_out, result)
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2))
...
def _execute_release_portfolio_governance_attestation_transparency_acknowledgement(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_attestation_transparency_acknowledgement_store(transparency_store=transparency_store)
    portfolio_id = args.portfolio_id
    result: JsonDocument = {"ok": True, "portfolio_id": portfolio_id, "profile": args.profile}
    ...
    if args.import_response:
        payload: JsonDocument = {"profile": args.profile}
...
def _execute_public_trust_center_publication(argv: list[str]) -> None:
    ...
    pass
    pass
    anchor_store = _trust_stores.public_trust_center_anchor_registry_store(trust_center_store=trust_store)
    anchor_transparency_store = _trust_stores.public_trust_center_anchor_transparency_store(anchor_registry_store=anchor_store)
    distribution_kit_store = _trust_stores.public_trust_center_distribution_kit_store(trust_center_store=trust_store, anchor_registry_store=anchor_store, anchor_transparency_store=anchor_transparency_store)
    acceptance_store = _trust_stores.public_trust_center_distribution_kit_acceptance_store(distribution_kit_store=distribution_kit_store)
    board_store = _trust_stores.public_trust_center_acceptance_board_store(acceptance_store=acceptance_store)
    store = _trust_stores.public_trust_center_publication_store(
    ...
    result: JsonDocument = {"ok": True, "center_id": args.center_id, "channel_id": args.channel_id}
    ...
    if args.export:
        ...
        publication_id = str(_as_document(result.get("manifest")).get("publication_id") or publication_id or "")
    if args.zip:
        result["zip"] = store.build_publication_zip(args.center_id, args.channel_id, publication_id)
        publication_id = str(_as_document(result.get("zip")).get("publication_id") or publication_id or "")
    if args.verify:
    ...
    if args.json:
        ...
        if "verification" in result:
            print_public_trust_center_publication_verification_report(_as_document(result.get("verification")))
        elif "mirror_verification" in result:
            print_public_trust_center_publication_verification_report(_as_document(result.get("mirror_verification")))
        else:
