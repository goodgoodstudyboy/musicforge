...
def _execute_public_trust_center_publication_monitor(argv: list[str]) -> None:
    ...
    store = _trust_stores.public_trust_center_publication_monitoring_store(publication_store=publication_store)
    ...
    if args.json:
        ...
        if "verification" in result:
            print_public_trust_center_publication_monitoring_verification_report(_as_document(result.get("verification")))
        else:
...
def _execute_trust_operations_hub_part_01(argv: list[str], _split_state):
    ...
    _split_state['store'] = _trust_stores.trust_operations_hub_store()
...
def _execute_trust_operations_assurance_watch(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    hub_store = _trust_stores.trust_operations_hub_store()
    assurance_store = _trust_stores.trust_operations_assurance_store(hub_store=hub_store)
    store = _trust_stores.trust_operations_assurance_watch_store(assurance_store=assurance_store, hub_store=hub_store)
    result: JsonDocument = {"ok": True, "schedule_id": args.schedule_id}
    source_payload = _trust_operations_assurance_watch_source_payload(args)
    schedule_patch: JsonDocument = {}
    if args.hub_id:
        scope = _as_document(schedule_patch.get("scope"))
        scope["hub_ids"] = [args.hub_id]
        schedule_patch["scope"] = scope
    if args.interval_days is not None or args.grace_days is not None:
        cadence = _as_document(schedule_patch.get("cadence"))
        if args.interval_days is not None:
            cadence["interval_days"] = args.interval_days
        if args.grace_days is not None:
            cadence["grace_days"] = args.grace_days
        schedule_patch["cadence"] = cadence
    if args.write_schedule:
        result["schedule"] = store.write_schedule({"schedule_id": args.schedule_id, **schedule_patch})
    if args.list:
        result["queues"] = normalize_json_value(store.list_queues(args.schedule_id))
    if args.refresh:
        refresh_payload: dict[str, object] = dict(source_payload)
        if args.queue_id:
    ...
    if args.export:
        ...
        result["manifest"] = store.export_watch(args.queue_id, dict(source_payload))
    if args.zip:
        if not args.queue_id:
            raise ValueError("--queue-id is required for --zip unless --refresh was used.")
        result["zip"] = store.build_watch_zip(args.queue_id, dict(source_payload))
    if args.verify:
        if not args.queue_id:
    ...
    if not any([args.write_schedule, args.list, args.refresh, args.export, args.zip, args.verify]):
        result["summary"] = normalize_json_value(
            store.summary(args.queue_id)
            if args.queue_id
            else {"schedule": store.read_schedule(args.schedule_id), "queues": store.list_queues(args.schedule_id)}
        )
    if args.report_out is not None:
        write_interface_document(args.report_out, result)
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_assurance_watch_verification_report(_as_document(result.get("verification")))
        else:
...
def _execute_trust_operations_assurance_watch_signoff(argv: list[str]) -> None:
    ...
    hub_store = _trust_stores.trust_operations_hub_store()
    assurance_store = _trust_stores.trust_operations_assurance_store(hub_store=hub_store)
    watch_store = _trust_stores.trust_operations_assurance_watch_store(assurance_store=assurance_store, hub_store=hub_store)
    store = _trust_stores.trust_operations_assurance_watch_signoff_store(watch_store=watch_store, assurance_store=assurance_store, hub_store=hub_store)
    result: JsonDocument = {"ok": True, "queue_id": args.queue_id}
    source_payload = {
    ...
    if args.json:
        ...
        if "verification" in result:
            print_trust_operations_assurance_watch_signoff_verification_report(_as_document(result.get("verification")))
        else:
            print(json.dumps(result.get("summary") or {"status": "ok", "queue_id": args.queue_id}, ensure_ascii=False, indent=2))
    raise SystemExit(0)
