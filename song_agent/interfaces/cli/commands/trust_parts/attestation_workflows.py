...
def _execute_release_portfolio_governance_attestation(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_attestation_store(portfolio_store=portfolio_store, final_board_store=final_board_store, evidence_vault_store=vault_store)
    portfolio_id = args.portfolio_id
    payload = {"profile": args.profile, "require_no_force": args.require_no_force}
    result: JsonDocument = {"ok": True, "portfolio_id": portfolio_id, "profile": args.profile}
    if args.refresh:
...
def _execute_release_portfolio_governance_attestation_registry(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_attestation_registry_store(attestation_store=attestation_store)
    portfolio_id = args.portfolio_id
    ...
    registry = _document_or(result.get("registry"), store.read_registry(portfolio_id, profile=args.profile, default={}))
...
def _execute_release_portfolio_governance_attestation_portal(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_attestation_portal_store(registry_store=registry_store, attestation_store=attestation_store)
    portfolio_id = args.portfolio_id
    payload = {"profile": args.profile}
    result: JsonDocument = {"ok": True, "portfolio_id": portfolio_id, "profile": args.profile}
    if args.refresh:
...
def _execute_release_portfolio_governance_attestation_portal_review(argv: list[str]) -> None:
    ...
    store = _trust_stores.release_portfolio_governance_attestation_portal_review_store(portal_store=portal_store)
    portfolio_id = args.portfolio_id
    result: JsonDocument = {"ok": True, "portfolio_id": portfolio_id, "profile": args.profile}
    ...
    if args.responses:
        result.update({"responses": normalize_json_value(store.list_responses(portfolio_id, profile=args.profile))})
