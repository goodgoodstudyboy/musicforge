...
from song_agent.interfaces.bootstrap.cli import stores as _delivery_stores
...
def _execute_release_operations_runbook(argv: list[str]) -> None:
    ...
    release_store = _delivery_stores.release_store()
    distribution_store = _delivery_stores.distribution_store(release_store)
    submission_store = _delivery_stores.submission_store(release_store, distribution_store)
    evidence_store = _delivery_stores.submission_evidence_store(submission_store)
    operations_store = _delivery_stores.release_operations_store(release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    store = _delivery_stores.release_operations_runbook_store(operations_store=operations_store, release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    result: JsonDocument = {"ok": True, "release_id": args.release_id}
    if args.list:
        runbooks = store.list_runbooks(args.release_id, include_archived=True)
        result.update({"runbooks": normalize_json_value(runbooks), "summary": {"count": len(runbooks)}})
    elif args.create:
...
def _execute_release_operations_signoff(argv: list[str]) -> None:
    ...
    evidence_store = _delivery_stores.submission_evidence_store(submission_store)
    operations_store = _delivery_stores.release_operations_store(release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    runbook_store = _delivery_stores.release_operations_runbook_store(operations_store=operations_store, release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    store = _delivery_stores.release_operations_signoff_store(operations_store=operations_store, runbook_store=runbook_store, release_store=release_store)
    result: JsonDocument = {"ok": True, "release_id": args.release_id}
    if args.reset:
...
def _execute_release_operations_audit(argv: list[str]) -> None:
    ...
    operations_store = _delivery_stores.release_operations_store(release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    runbook_store = _delivery_stores.release_operations_runbook_store(operations_store=operations_store, release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    signoff_store = _delivery_stores.release_operations_signoff_store(operations_store=operations_store, runbook_store=runbook_store, release_store=release_store)
    store = _delivery_stores.release_operations_audit_store(operations_store=operations_store, runbook_store=runbook_store, signoff_store=signoff_store, release_store=release_store)
    result: JsonDocument = {"ok": True, "release_id": args.release_id}
    if args.refresh:
    ...
    if args.entries:
        ...
        result.update({"entries": normalize_json_value(entries), "entry_summary": {"entry_count": len(entries)}})
    if args.graph:
...
def _execute_release_operations_reviewer_pack(argv: list[str]) -> None:
    ...
    runbook_store = _delivery_stores.release_operations_runbook_store(operations_store=operations_store, release_store=release_store, distribution_store=distribution_store, submission_store=submission_store, submission_evidence_store=evidence_store)
    signoff_store = _delivery_stores.release_operations_signoff_store(operations_store=operations_store, runbook_store=runbook_store, release_store=release_store)
    audit_store = _delivery_stores.release_operations_audit_store(operations_store=operations_store, runbook_store=runbook_store, signoff_store=signoff_store, release_store=release_store)
    store = _delivery_stores.release_operations_reviewer_pack_store(audit_store=audit_store, signoff_store=signoff_store, release_store=release_store)
    result: JsonDocument = {"ok": True, "release_id": args.release_id}
    if args.refresh:
        report = store.refresh(args.release_id)
...
def _execute_release_encode(argv: list[str]) -> None:
    ...
    args = parser.parse_args(raw_args[1:])
    project_store = _delivery_stores.project_store()
    release_store = _delivery_stores.release_store(project_store=project_store)
    profile_store = _delivery_stores.audio_encoding_profile_store(release_store.root.parent / "audio-encoding-profiles")
    store = _delivery_stores.audio_encoding_store(release_store, project_store=project_store, profile_store=profile_store)
