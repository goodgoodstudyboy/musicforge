...
from song_agent.interfaces.cli.commands.program_parts.unified_command_center_reviewer_decision_board import (
...
def _release_train_handoff_payload_from_args(args: Namespace) -> JsonDocument:
    return {
...
def print_release_operations_result(result: JsonDocument) -> None:
    ...
    if result.get("acceptance_board_signoff_archive_zip"):
        print(
            ...
            f"{_as_document(result.get('acceptance_board_signoff_archive_zip')).get('sha256')}"
        )
    if archive_verification:
        print(f"acceptance board signoff archive verify: {archive_verification.get('status')}")

def print_release_operations_runbook_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    verification = _as_document(result.get("verification_summary"))
...
def print_release_operations_signoff_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
...
def print_release_operations_archive_result(result: JsonDocument) -> None:
    manifest = _as_document(result.get("manifest"))
    verification = _as_document(result.get("verification_summary"))
    print("MusicForge release-operations-archive")
    print(f"release: {result.get('release_id') or manifest.get('release_id') or '-'}")
    if manifest:
        print(f"archive: {_as_document(manifest.get('summary')).get('status') or '-'}")
    if result.get("zip"):
        print(f"zip: {_as_document(result.get('zip')).get('filename')}")
    if verification:
        print(f"verify: {verification.get('status')}")

def print_release_operations_audit_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    verification = _as_document(result.get("verification_summary"))
...
def print_release_operations_reviewer_pack_result(result: JsonDocument) -> None:
    summary = _as_document(result.get("summary"))
    verification = _as_document(result.get("verification_summary"))
