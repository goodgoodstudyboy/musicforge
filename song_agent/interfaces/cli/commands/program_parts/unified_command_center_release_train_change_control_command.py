...
from song_agent.interfaces.cli.commands.delivery_parts.release_train_handoff_payload_from_args import _release_train_handoff_payload_from_args
from song_agent.interfaces.cli.commands.delivery_parts.verify_release_and_adapters import _release_train_lifecycle_payload_from_args
...
def _run_unified_command_center_release_train_change_control_command(args: argparse.Namespace) -> JsonDocument:
    pass
    pass
    pass

    train_store = _program_stores.unified_command_center_release_train_store()
    store = _program_stores.unified_command_center_release_train_change_control_store(train_store)
    ...
    if args.action == "status":
        report: JsonDocument = store.refresh_report(args.train_id) if store.change_dir(args.train_id).exists() else {"status": "not_configured", "summary": {}}
        return normalize_json_document({"ok": report.get("status") != "failed", "report": report, "summary": report.get("summary", {}), "status": report.get("status")})
    ...
    if args.action == "verify":
        ...
        return normalize_json_document({"ok": report.get("status") == "passed", "verification": report, "summary": report.get("summary", {}), "status": report.get("status")})
    raise ValueError("Unsupported unified-command-center-release-train-change-control command.")

def _run_unified_command_center_release_train_lifecycle_command(args: argparse.Namespace) -> JsonDocument:
    pass
    pass
    pass
    pass
    ...
    train_store = _program_stores.unified_command_center_release_train_store()
    change_store = _program_stores.unified_command_center_release_train_change_control_store(train_store)
    store = _program_stores.unified_command_center_release_train_lifecycle_store(train_store, change_store)
    if args.action == "status":
        report: JsonDocument = store.read_report(args.train_id) if store.report_path(args.train_id).exists() else {"status": "not_configured", "summary": {}}
        return normalize_json_document({"ok": report.get("status") != "failed", "report": report, "summary": report.get("summary", {}), "status": report.get("status")})
    payload = _release_train_lifecycle_payload_from_args(args)
    if args.action == "refresh":
        report = store.refresh_report(args.train_id, payload)
        return normalize_json_document({"ok": report.get("status") == "passed", "report": report, "summary": report.get("summary", {}), "status": report.get("status")})
    if args.action == "export":
...
def _run_unified_command_center_release_train_handoff_command(args: argparse.Namespace) -> JsonDocument:
    pass
    pass
    pass
    pass
    ...
    train_store = _program_stores.unified_command_center_release_train_store()
    change_store = _program_stores.unified_command_center_release_train_change_control_store(train_store)
    lifecycle_store = _program_stores.unified_command_center_release_train_lifecycle_store(train_store, change_store)
    store = _program_stores.unified_command_center_release_train_handoff_store(train_store, change_store, lifecycle_store)
...
def _run_unified_release_program_command(args: argparse.Namespace) -> JsonDocument:
    pass
...
def _unified_release_program_operations_payload_from_args(args: argparse.Namespace) -> JsonDocument:
    payload: JsonDocument = {
        "program_zip": normalize_json_value(getattr(args, "program_zip", None)),
        "program_verification_report": normalize_json_value(getattr(args, "program_verification_report", None)),
        "program_signoff_binding": normalize_json_value(getattr(args, "program_signoff_binding", None)),
        "external_evidence_manifest": normalize_json_value(getattr(args, "external_evidence_manifest", None)),
    }
    for name in ("change_request_id", "change_type", "reason", "requested_by", "approved_by", "role", "reset_by", "allowed_actions"):
        value = getattr(args, name, None)
        if value is not None:
            payload[name] = normalize_json_value(value)
    return payload

def _run_unified_release_program_operations_command(args: argparse.Namespace) -> JsonDocument:
    pass
